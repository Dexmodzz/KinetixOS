...
static void dwindlegeom(Client *c, int x, int y, int w, int h);
...
static const char dwmdir[] = "kinetixos";
...
/* Default (fallback) config paths: ~/.local/share/kinetixos/config/ */
...
createmon(void)
{
	...
	copystr(m->ltsymbol, sizeof m->ltsymbol, layouts[defaultlayout].symbol);
...
/* Recursively split the given rectangle between c and the tiled clients
 * that follow it: c takes mfact of the longer axis, the remainder recurses
 * for the rest of the clients. This mirrors Hyprland's default dwindle
 * layout instead of dwm's master/stack tile(). */
void
dwindlegeom(Client *c, int x, int y, int w, int h)
{
	Client *next = nexttiled(c->next);
	int w1, h1;

	if (!next) {
		resize(c, x + gappx / 2, y + gappx / 2,
			w - gappx - 2 * c->bw, h - gappx - 2 * c->bw, 0);
		return;
	}

	if (w >= h) {
		w1 = (int)(w * c->mon->mfact);
		resize(c, x + gappx / 2, y + gappx / 2,
			w1 - gappx - 2 * c->bw, h - gappx - 2 * c->bw, 0);
		dwindlegeom(next, x + w1, y, w - w1, h);
	} else {
		h1 = (int)(h * c->mon->mfact);
		resize(c, x + gappx / 2, y + gappx / 2,
			w - gappx - 2 * c->bw, h1 - gappx - 2 * c->bw, 0);
		dwindlegeom(next, x, y + h1, w, h - h1);
	}
}
...
dwindle(Monitor *m)
{
	...
	if (!(c = nexttiled(m->clients)))
		return;
	dwindlegeom(c, m->wx, m->wy, m->ww, m->wh);
}
...
setup_inotify(void)
{
	...
	/* User-editable config: ${XDG_CONFIG_HOME:-$HOME/.config}/kinetixos/ */
	if (!pathjoin(toml_config_dir, sizeof(toml_config_dir),
	              config_home, "kinetixos")
	...
	/* Default config: ${XDG_DATA_HOME:-$HOME/.local/share}/kinetixos/config/ */
...
updatestatus(void)
{
	if (!gettextprop(root, XA_WM_NAME, stext, sizeof(stext))) {
		strcpy(stext, "kinetixos:"VERSION);
