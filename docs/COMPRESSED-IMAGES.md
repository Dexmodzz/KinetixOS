...
KinetixOS does not publish prebuilt images; build them locally as described below.
...
scripts/build-dwm-fedora-installer-iso.sh \
  ...
  --output "$HOME/tmp/dwm-compressed/artifacts/kinetixos.iso" \
  --variant standard
```

Repeat both commands with `--variant nvidia` and distinct `nvidia.tar.zst` and
`kinetixos-nvidia.iso` outputs. Standard and NVIDIA archives are not
...
RPM and Flatpak manifests under `/usr/share/kinetixos-image/`.
...
investigated separately; see the in-progress Phase 8 evidence.
