...
8. Run `scripts/run-tests make release-check` and confirm the artifact is named
   `release/kinetixos-VERSION.tar.gz`.
...
```sh
scripts/kinetixos-release --version v0.6.1 --iso ~/Downloads/kinetixos.iso --notes RELEASE_NOTES.md
```
...
[COMPRESSED-IMAGES.md](COMPRESSED-IMAGES.md). Build success alone does not
qualify installation or first boot.
...
scripts/build-dwm-fedora-installer-iso.sh \
  --input ~/Downloads/Fedora-Server-netinst-x86_64-44-1.7.iso \
  --output release/kinetixos-nvidia.iso \
  --variant nvidia
```

Both Kickstart profiles share the Fedora, RPM Fusion, Brave, and MWT repository
declarations used by this project. On x86_64, both also install the gaming
packages, with Gamescope from the official Fedora repositories; other
architectures omit those x86-only gaming packages. The ISO builder selects the matching standard or
...
Publish the standard and NVIDIA images at `iso/kinetixos.iso` and
`iso/kinetixos-nvidia.iso`, with matching `iso/SHA256SUMS`,
