...
The first card in **Settings > System** checks the KinetixOS desktop against
...
- `${PREFIX}/share/kinetixos/desktop-install.json`: root-owned revision,
  system-file hashes, destinations, and dependency allowlist.
- `${XDG_STATE_HOME:-$HOME/.local/state}/kinetixos/desktop-update/installed.json`:
...
- `/var/lib/kinetixos/desktop-updates/<id>/`: private root-owned system backup
