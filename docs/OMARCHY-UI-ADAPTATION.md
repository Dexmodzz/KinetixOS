...
KinetixOS borrows the visual grammar of Omarchy's Quickshell menus without
...
session policy continue to belong to KinetixOS.

This is a design influence, not a shell transplant. Omarchy is MIT licensed;
the implementation here uses independently written KinetixOS QML backed by the
...
New menus must use the existing KinetixOS popup and panel primitives. They
...
existing KinetixOS surface directly. Type to search commands and applications
across the catalog.
