...
The generation is SHA-256 over the ASCII prefix `kinetixos-regional-preview-v1`
...
digest of the ASCII prefix `kinetixos-update-plan-v1`, followed first by the
...
- An install-all confirmation captures the snapshot generation and displays
  ...
  begins with the ASCII prefix `kinetixos-update-observed-v1` and then encodes
...
- The journal lives under
  `${XDG_STATE_HOME:-$HOME/.local/state}/kinetixos/system-management/`, is mode
...
in P6-DELEGATE-UI-EVIDENCE.md. Regional confirmation,
...
recorded in P6-UPDATE-EVIDENCE.md. This does not replace
