...
Replaced profiles are backed up in the KinetixOS `display-profile-backups`
...
`rollback-system` actions. Named layouts live under the KinetixOS XDG config
...
`90-kinetixos-display.conf` fragment after a separate confirmation and polkit
