...
description: Use the KinetixOS Control Center for system health, quick actions, appearance, defaults, startup apps, and panel widgets.
...
`~/.config/kinetixos/panel-widgets.conf` state and apply to every monitor and
...
The managed copy under `~/.local/share/kinetixos` is never elevated itself. If
...
```bash
quickshell ipc --path "${XDG_DATA_HOME:-$HOME/.local/share}/kinetixos/config/quickshell/shell.qml" call controlcenter toggle
```
