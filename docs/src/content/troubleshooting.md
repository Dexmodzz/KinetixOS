...
description: Resolve common KinetixOS startup, panel, application, theme, keybind, multi-monitor, NVIDIA, suspend, and compositor issues.
...
application setup from an installed KinetixOS checkout, run:
...
- Confirm `themes.toml` is at `~/.config/kinetixos/themes.toml`
...
- Bad persistent layout: run `dwm-display-setup rollback`, then log out and
  ...
  `/etc/X11/xorg.conf.d/90-kinetixos-display.conf` if Xorg cannot start.
...
- Open an issue: [github.com/Dexmodzz/KinetixOS/issues](https://github.com/Dexmodzz/KinetixOS/issues)
