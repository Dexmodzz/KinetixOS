---
title: Configuration
description: Customize KinetixOS defaults, live-reloading hotkeys, window rules, and XDG autostart behavior.
...
KinetixOS keeps user configuration under
`${XDG_CONFIG_HOME:-$HOME/.config}/kinetixos/`. Hotkeys and themes
...
`${XDG_CONFIG_HOME:-$HOME/.config}/kinetixos/display-profiles`. Use
...
`/etc/X11/xorg.conf.d/90-kinetixos-display.conf`; existing Xorg files are not
...
only the root-owned helper under `${PREFIX}/libexec/kinetixos/` may update the
...
`${XDG_CONFIG_HOME:-$HOME/.config}/kinetixos/input-settings.conf`. The
...
`${XDG_CONFIG_HOME:-$HOME/.config}/kinetixos/notification-settings.json`.
...
event-driven session listener. Until `power.conf` exists, KinetixOS leaves any
