---
title: Theming
description: Change KinetixOS colors, borders, wallpaper, fonts, text scale, cursor, icons, and application appearance.
navLabel: Theming
...
The setting owns only `font.conf` under the KinetixOS XDG configuration
...
use a compatible newer KinetixOS version rather than repairing them.
...
the KinetixOS appearance state directory for the existing theme recovery flow.
