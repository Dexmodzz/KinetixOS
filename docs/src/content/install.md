...
description: Install the complete KinetixOS Fedora X11 desktop from an image or onto an existing Fedora system.
...
> **KinetixOS is Fedora-only.** Fedora Linux with Xorg is required for every
> supported installation, package, test, and release path.

## Fedora ISO (Recommended for a New Installation)

KinetixOS does not publish prebuilt ISO images yet. Build the standard or
NVIDIA offline image locally from the repository; see `docs/COMPRESSED-IMAGES.md`
and `docs/RELEASING.md`. The resulting image installs a complete Fedora 44
x86_64 desktop from a compressed system image. Packages are included, so
installation works without an Internet connection or software selection.
...
The image build writes a `SHA256SUMS` file next to the ISO. In that directory,
run:
...
Your ISO must report `OK`; do not continue with a mismatch.
...
The installer separately asks before enabling RPM Fusion nonfree for Steam;
Gamescope comes from the official Fedora repositories. Declining skips the
...
- `full`: `recommended` plus optional extras such as Thunar with SMB-share
  ...
  The installer enables RPM Fusion nonfree for Steam, installs Gamescope from
  Fedora, then adds the invoking user to the `gamemode`
...
[Desktop updates and recovery](https://github.com/Dexmodzz/KinetixOS/blob/main/docs/DESKTOP-UPDATES.md) for the update
...
```bash
repo_dir=$(pwd -P)
backup_dir=$(mktemp -d "$HOME/kinetixos-backup.XXXXXX") || exit 1
...
restore_dir=$(mktemp -d "$HOME/kinetixos-restored.XXXXXX") || exit 1
...
the root-owned helper installed at `${PREFIX}/libexec/kinetixos/`. Profiles are
stored under
`${XDG_CONFIG_HOME:-$HOME/.config}/kinetixos/display-profiles/`. No move is
...
`${XDG_CONFIG_HOME:-$HOME/.config}/kinetixos/input-settings.conf`. Set
