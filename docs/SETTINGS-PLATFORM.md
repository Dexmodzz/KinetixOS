...
`${XDG_CONFIG_HOME:-$HOME/.config}/kinetixos/self-heal.path` (one path, no shell
...
`~/.config/kinetixos/display-profile-backups/` (respecting `XDG_CONFIG_HOME`).
An existing `~/.autorandr` directory takes precedence, matching autorandr;
its backups are stored in `~/kinetixos/display-profile-backups/`.
...
Named KinetixOS layouts and privileged **Use at next login** remain separate
...
`/etc/X11/xorg.conf.d/90-kinetixos-display.conf`. Root-container tests cover
