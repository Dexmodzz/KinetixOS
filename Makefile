...
PRIVILEGED_HELPER_DIR = ${PREFIX}/libexec/kinetixos

RELEASE_NAME = kinetixos-${VERSION}
...
install:
	...
	/usr/bin/python3 -I scripts/dwm-desktop-update guard-system-install --destdir "${DESTDIR}" \
		--owner "${OWNER}" --user-state "${XDG_STATE_HOME}/kinetixos/desktop-update" -- $(MAKE) install-files
...
install-system-files:
	...
	install -d -m755 "${DESTDIR}${PREFIX}/share/kinetixos"
	...
	if [ -z "${DESTDIR}" ] && [ "$$(id -u)" -eq 0 ]; then \
		echo "==> Migrating legacy dwm-titus system files..."; \
		scripts/migrate-legacy-name.sh --system "${PREFIX}"; \
	fi
...
install-user:
	/usr/bin/python3 -I scripts/dwm-desktop-update guard-user-install "${XDG_STATE_HOME}/kinetixos/desktop-update" -- $(MAKE) install-user-files

install-user-files:
	@test -n "${USER_HOME}" || { echo "USER_HOME could not be determined." >&2; exit 1; }
	@echo "==> Migrating legacy dwm-titus user directories..."
	HOME="${USER_HOME}" XDG_CONFIG_HOME="${XDG_CONFIG_HOME}" XDG_DATA_HOME="${XDG_DATA_HOME}" \
		XDG_STATE_HOME="${XDG_STATE_HOME}" scripts/migrate-legacy-name.sh
	...
	mkdir -p ${CFG_DIR}/kinetixos
	test -f ${CFG_DIR}/kinetixos/hotkeys.toml || install -Dm644 config/hotkeys.toml ${CFG_DIR}/kinetixos/hotkeys.toml
	test -f ${CFG_DIR}/kinetixos/themes.toml  || install -Dm644 config/themes.toml  ${CFG_DIR}/kinetixos/themes.toml
	test -f ${CFG_DIR}/kinetixos/window-rules.toml || install -Dm644 config/window-rules.toml ${CFG_DIR}/kinetixos/window-rules.toml
...
uninstall-files:
	...
	rm -f "${DESTDIR}${PREFIX}/share/kinetixos/desktop-install.json"
...
check-thunar-sidebar:
	tests/test-thunar-sidebar.sh

check-hdmi-watch:
	tests/test-dwm-hdmi-watch.sh

check-migrate-legacy-name:
	tests/test-migrate-legacy-name.sh
...
check-kickstart:
	for ks in dwm-fedora.ks dwm-fedora-nvidia.ks; do \
		...
		awk 'BEGIN { bad = 0 } \
			/^[[:space:]]*#/ { next } \
			/(^user[[:space:]]|^rootpw[[:space:]]|--hostname=|\/home\/nicola|America\/Chicago|^initial-setup$$|^firstboot[[:space:]]+--enable)/ { \
...
check-install-manifest: all
	...
	{ \
		printf '%s\n' \
			pre-existing \
			usr/bin/dwm \
			usr/libexec/kinetixos/dwm-settings-display-root \
			usr/libexec/kinetixos/dwm-desktop-update-root \
			usr/share/kinetixos/desktop-install.json \
		...
		printf '%s\n' \
			usr/share/licenses/kinetixos/capitaine-cursors/COPYING; \
	} | sort > "$$expected"; \
	find "$$stage" \( -type f -o -type l \) -printf '%P\n' | sort > "$$actual"; \
	cmp "$$expected" "$$actual"; \
	...
	test -x "$$stage/usr/libexec/kinetixos/dwm-settings-display-root"; \
...
check: check-picom check-picom-xvfb
	...
	$(MAKE) check-thunar-sidebar
	$(MAKE) check-hdmi-watch
	$(MAKE) check-migrate-legacy-name
...
.PHONY: install-files install-user-files install-system-files clean all check check-desktop-update check-picom check-picom-xvfb check-accessibility check-appearance check-phase5-optional-components check-build-config check-build-deps check-default-apps check-thunar-sidebar check-hdmi-watch check-migrate-legacy-name check-xdg-autostart check-dev-sync-install \
	check-cursor-reload \
	check-test-runner \
