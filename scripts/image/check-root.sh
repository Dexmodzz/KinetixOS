...
rm -f /etc/sudoers.d/90-kinetixos-install /etc/kinetixos-factory-target
...
shared_assets=(/usr/share/kinetixos-image /usr/local/share/fonts/kinetixos
...
source /usr/share/kinetixos-image/scripts/dwm-packages.sh
mapfile -t packages < <({
...
[[ -z $(find /usr/share/kinetixos-image \( -name '.env' -o -name '.env.*' -o -name '.envrc' \) -print -quit) ]]
missing=0
...
((missing == 0))
sha256sum --check /usr/share/kinetixos-image/tool-sha256.txt
...
[[ ! -e /etc/sudoers.d/90-kinetixos-install ]]
