...
if [[ -z "$THEME_RUNTIME_BASE" ]]; then
	THEME_RUNTIME_BASE=/tmp/kinetixos-$UID
...
# ── Locate themes.toml ────────────────────────────────────────────────────────
THEMES_FILE="${DWM_APPEARANCE_THEMES_FILE:-${XDG_CONFIG_HOME:-$HOME/.config}/kinetixos/themes.toml}"
MANAGED_THEMES_FILE="${DWM_APPEARANCE_MANAGED_THEMES_FILE:-${XDG_DATA_HOME:-$HOME/.local/share}/kinetixos/config/themes.toml}"
...
if [[ $RUNTIME_ONLY_EXPLICIT == false ]]; then
	...
	if [[ $AUTOMATIC_APPLY == 1 ]]; then
		THEME_SUPPRESS_FILE=$THEME_STATE_HOME/kinetixos/appearance/integration-suppress
	...
	THEME_TRANSACTION_FILE=$THEME_STATE_HOME/kinetixos/appearance/integration-transaction
...
PERSONALIZATION_FILE="${XDG_CONFIG_HOME:-$HOME/.config}/kinetixos/personalization.conf"
...
# Change the theme in ~/.config/kinetixos/themes.toml instead.
...
XSETTINGSD_CONFIG="${XDG_CONFIG_HOME:-$HOME/.config}/kinetixos/xsettingsd.conf"
if [[ $RUNTIME_ONLY == 0 && $LIVE_ONLY == 0 ]]; then
...
CURSOR_XRESOURCES="${XDG_CONFIG_HOME:-$HOME/.config}/kinetixos/cursor.Xresources"
if [[ $RUNTIME_ONLY == 0 && $LIVE_ONLY == 0 ]]; then
...
THEME_ENV_FILE="${XDG_CONFIG_HOME:-$HOME/.config}/kinetixos/theme-env.sh"
if [[ $RUNTIME_ONLY == 0 && $LIVE_ONLY == 0 ]]; then
