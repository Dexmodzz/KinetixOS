#!/bin/sh
# KinetixOS autostart — single unified autostart script
...
start_detached_display_command_once() {
	...
	if [ -n "${XDG_RUNTIME_DIR:-}" ] && [ -d "$XDG_RUNTIME_DIR" ] &&
		...
		display_lock_dir=$XDG_RUNTIME_DIR/kinetixos
...
THEME_ENV="${XDG_CONFIG_HOME:-$HOME/.config}/kinetixos/theme-env.sh"
# shellcheck disable=SC1090
[ -f "$THEME_ENV" ] && . "$THEME_ENV"

# Make a connected external monitor the only, primary display before the shell
# and applications start, even when the laptop lid is closed at login.
hdmi_watch=dwm-hdmi-watch
if ! command -v "$hdmi_watch" >/dev/null 2>&1; then
	case $0 in
	*/*) hdmi_watch=${0%/*}/dwm-hdmi-watch ;;
	esac
fi
if command -v "$hdmi_watch" >/dev/null 2>&1; then
	timeout --signal=TERM --kill-after=1 8 \
		"$hdmi_watch" --apply >/dev/null 2>&1 ||
		printf '%s\n' 'kinetixos: external monitor layout could not be applied' >&2
fi
...
if [ -n "$xsettings_helper" ]; then
	timeout --signal=TERM --kill-after=1 8 \
		...
		printf '%s\n' 'kinetixos: managed X11 text scaling is unavailable' >&2
fi

# Seed the Thunar side pane once per account from the localized XDG folders.
# It needs the session bus for xfconf; a failure retries on the next login.
case $0 in
*/*)
	thunar_sidebar_seeder=${0%/*}/seed-thunar-sidebar.sh
	if [ -f "$thunar_sidebar_seeder" ]; then
		timeout --signal=TERM --kill-after=1 8 \
			sh "$thunar_sidebar_seeder" >/dev/null 2>&1 ||
			printf '%s\n' 'kinetixos: Thunar side pane seeding failed' >&2
	fi
	;;
esac
...
if [ -f "$QUICKSHELL_CONFIG" ]; then
	...
	*/*)
	...
	if [ -n "$quickshell_check" ] && "$quickshell_check"; then
		...
		printf '%s\n' 'kinetixos: compatible Quickshell 0.3.0 or Fedora 44 snapshot is required' >&2
	fi
...
# Follow monitor hotplug for this X session. The watcher is bound to its X
# server and holds a per-display lock, so it is started on every session
# instead of being skipped when a stale process from an old session exists.
start_detached "$hdmi_watch"
