...
Build and synchronize the current checkout with the live KinetixOS install.
The default mode:
...
display_root_helper_target=$prefix/libexec/kinetixos/dwm-settings-display-root
desktop_root_helper_target=$prefix/libexec/kinetixos/dwm-desktop-update-root
...
verify_install() {
	...
	if [ "$verify_privileged_helper_trust" -eq 1 ]; then
		if ! python3 "$repo_dir/scripts/dwm-desktop-update" verify-trust \
			"$prefix/share/kinetixos/desktop-install.json"; then
			verification_failed=1
		fi
	...
	verify_file "$repo_dir/assets/cursors/COPYING" \
		"$data_root/licenses/kinetixos/capitaine-cursors/COPYING" \
		"cursor license"
	if ! HOME="$user_home" XDG_CONFIG_HOME="$config_home" XDG_DATA_HOME="$xdg_data_home" \
		...
		verify-receipts "$repo_dir" "$prefix/share/kinetixos/desktop-install.json"; then
		verification_failed=1
	fi
...
backup_live_install() {
	...
	backup_parent=$state_home/kinetixos/live-update-backups
	...
	if [ -e "$data_dir" ]; then
		tar -C "$(dirname "$data_dir")" -cpf \
			"$backup_dir/kinetixos-data.tar" "$(basename "$data_dir")"
	fi
	...
	add_system_backup_path "$prefix/share/kinetixos/desktop-install.json"
	for cursor_source in "$repo_dir"/assets/cursors/Capitaine-Cursors*; do
		[ -d "$cursor_source" ] || continue
		add_system_backup_path "$data_root/icons/${cursor_source##*/}"
	done
	add_system_backup_path "$data_root/licenses/kinetixos/capitaine-cursors/COPYING"
