    ...
    with tempfile.TemporaryDirectory(prefix='dwm-factory.', dir=test_root) as temp:
        ...
        with (evidence / 'build.log').open('w') as log:
            ...
            ks += ('\n%post --interpreter=/bin/bash --erroronfail\nset -euo pipefail\nexec > /dev/ttyS0 2>&1\n'
                   f"printf '%s\\n' '{args.variant}' > /etc/kinetixos-factory-target\n"
                   'bash /home/imagebuilder/.local/share/kinetixos/scripts/image/prepare-factory.sh --factory-target\n%end\n')
            ...
            marker = run('guestfish', '--ro', '-a', work / 'factory.qcow2', '-i', 'cat',
                         '/etc/kinetixos-image', env=env, capture_output=True, text=True).stdout
