# KinetixOS Project Specification

## 1. Product Definition

KinetixOS is a Fedora-only desktop environment for X11. It combines a small,
...
- Build and run the complete KinetixOS desktop on supported Fedora releases.
...
Quickshell configuration is a managed shell-layer artifact owned by KinetixOS.
...
the kinetixos-managed Quickshell directory. Quickshell integrations must use
...
and only one shell provider should run in the KinetixOS session. On X11, the
...
When development is performed on a machine running KinetixOS, the
...
2. Install the system files and refresh
   `${XDG_DATA_HOME:-$HOME/.local/share}/kinetixos/` and the managed
...
- Failed system and user services, time synchronization, networking, audio,
  and the KinetixOS desktop session.
...
- Required and optional KinetixOS commands, libraries, configuration, and the
...
```text
${XDG_CONFIG_HOME:-$HOME/.config}/kinetixos/
${XDG_DATA_HOME:-$HOME/.local/share}/kinetixos/
```
...
KinetixOS 0.1.0 inherits the completed desktop phases. Physical NVIDIA, Secure
Boot and suspend limitations remain explicit; completion does not establish untested hardware.
...
- On a development machine running KinetixOS, the checkout has been
