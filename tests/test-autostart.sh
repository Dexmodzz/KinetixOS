...
# Keep the monitor layout helper away from the host X server and udev.
cat >"$work/bin/dwm-hdmi-watch" <<'EOF'
#!/bin/sh
exit 0
EOF

# Keep the Thunar side pane seeder away from the host xfconf daemon.
...
run_status_optional_lock_case() {
	...
	DISPLAY=:104 HOME=$home TEST_STATE=$state PATH="$work/bin:/usr/bin:/bin" \
		...
		TEST_CHMOD_FAILURE_TARGET="$runtime/kinetixos" \
		sh "$repo_dir/scripts/autostart.sh"
	wait_for_marker "$state/dwm-status.running"
