...
with tempfile.TemporaryDirectory(prefix="desktop-integration-", dir="/opt") as temporary:
    ...
    user("make", "clean", "all")
    journal = Path("/var/lib/kinetixos/desktop-updates") / ("e" * 32) / "journal.json"
    ...
    data = Path("/home/desktop-test/.local/share/kinetixos")
    state = Path(env["XDG_STATE_HOME"]) / "kinetixos/desktop-update"
    (config / "kinetixos/themes.toml").write_text("personal-theme-marker")
    ...
    before_manifest = json.loads((prefix / "share/kinetixos/desktop-install.json").read_text())
    ...
    helper = str(prefix / "libexec/kinetixos/dwm-desktop-update-root")
...
def trust(path):
    ...
    update.SOURCE = "https://github.com/Dexmodzz/KinetixOS.git"
...
manifest = Path(sys.argv[2]) / "share/kinetixos/desktop-install.json"
...
''')
        ...
        after_manifest = json.loads((prefix / "share/kinetixos/desktop-install.json").read_text())
        ...
        assert (config / "kinetixos/themes.toml").read_text() == "personal-theme-marker"
        ...
        user("python3", prefix / "bin/dwm-desktop-update", "verify-receipts", source,
             prefix / "share/kinetixos/desktop-install.json")
