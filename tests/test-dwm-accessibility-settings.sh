...
[ "$(stat -c %a "$config/kinetixos/accessibility.conf")" = 600 ]
...
chmod 640 "$config/kinetixos/accessibility.conf"
run_helper set contrast standard >/dev/null
[ "$(stat -c %a "$config/kinetixos/accessibility.conf")" = 640 ]

reset_action=$(run_helper reset)
printf '%s\n' "$reset_action" | grep -Fqx 'result	reset	all	defaults'
grep -Fqx 'contrast	standard' "$config/kinetixos/accessibility.conf"
grep -Fqx 'motion	full' "$config/kinetixos/accessibility.conf"
...
rm "$config/kinetixos/accessibility.conf"
rmdir "$config/kinetixos"
for _ in $(seq 1 200); do
...
watch_pid=
mkdir "$config/kinetixos"

printf 'broken-protocol\n' >"$config/kinetixos/accessibility.conf"
...
grep -Fqx 'contrast	standard' "$config/kinetixos/accessibility.conf"
grep -Fqx 'motion	reduced' "$config/kinetixos/accessibility.conf"
...
grep -Fqx 'accessibility-settings-protocol	1	0' "$config/kinetixos/accessibility.conf"
grep -Fqx 'contrast	standard' "$config/kinetixos/accessibility.conf"
grep -Fqx 'motion	full' "$config/kinetixos/accessibility.conf"
...
grep -Fqx 'accessibility-settings-protocol	2	0' "$config/kinetixos/accessibility.conf"
cp "$config/kinetixos/accessibility.conf" "$work/future.before"
...
cmp "$work/future.before" "$config/kinetixos/accessibility.conf"
...
[ ! -e "$no_exchange_config/kinetixos/accessibility.conf" ]
...
printf '%s\n' "$exchange_unavailable_status" | grep -Fqx \
	'mutation	unavailable	Atomic accessibility state exchange is unavailable on the configuration filesystem'
if find "$config/kinetixos" -maxdepth 1 -name '.accessibility-exchange-*' -print -quit |
	grep -q .; then
...
grep -Fqx 'second last-moment edit' "$config/kinetixos/accessibility.conf"

target=$work/target
printf 'do not replace\n' >"$target"
rm "$config/kinetixos/accessibility.conf"
ln -s "$target" "$config/kinetixos/accessibility.conf"
...
printf 'accessibility-settings-protocol	1	0\ncontrast	standard\nmotion	full\n' \
	>"$config/kinetixos/accessibility.conf"
ln "$config/kinetixos/accessibility.conf" "$work/accessibility-hardlink.conf"
