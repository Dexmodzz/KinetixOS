...
mkdir -p "$work/bin" "$work/config/kinetixos" "$work/home" "$work/state"
: >"$work/actions.log"
...
write_initial_config() {
	cat >"$work/config/kinetixos/power.conf" <<'EOF'
...
lock_after=5
EOF
	chmod 600 "$work/config/kinetixos/power.conf"
...
write_initial_config
before=$(sha256sum "$work/config/kinetixos/power.conf")
...
[ "$(sha256sum "$work/config/kinetixos/power.conf")" = "$before" ]
...
grep -Fqx '# user power preferences' "$work/config/kinetixos/power.conf"
grep -Fqx 'custom_key=keep' "$work/config/kinetixos/power.conf"
grep -Fqx 'dpms_enabled=1' "$work/config/kinetixos/power.conf"
grep -Fqx 'dpms_timeout=900' "$work/config/kinetixos/power.conf"
[ "$(stat -c %a "$work/config/kinetixos/power.conf")" = 600 ]
if find "$work/config/kinetixos" -maxdepth 1 -name '.power.conf.*' -print -quit | grep -q .; then
...
write_initial_config
sed -i 's/^lock_enabled=0$/lock_enabled=1/' "$work/config/kinetixos/power.conf"
export DWM_POWER_TEST_LOCKER_NO_CONVERGE=1
...
write_initial_config
mv "$work/config/kinetixos/power.conf" "$work/power-target.conf"
ln -s "$work/power-target.conf" "$work/config/kinetixos/power.conf"
...
rm "$work/config/kinetixos/power.conf"
mv "$work/power-target.conf" "$work/config/kinetixos/power.conf"
