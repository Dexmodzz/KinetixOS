...
proc_root=$work/proc
mkdir -p "$config_home/kinetixos" "$config_home/dconf" "$config_home/fontconfig" \
	...
	"$state_root/kinetixos/appearance/wallpaper" \
...
config_dir=$(readlink -m -- "$XDG_CONFIG_HOME/kinetixos")
appearance_state_dir=$(readlink -m -- "${XDG_STATE_HOME:-$HOME/.local/state}/kinetixos/appearance")
...
grep -Fq -- "$state_root/kinetixos/appearance/wallpaper" "$watch_args"
