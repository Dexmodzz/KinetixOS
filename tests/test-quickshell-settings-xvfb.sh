...
start_quickshell() {
	env DISPLAY="$display" HOME="$home" XDG_CONFIG_HOME="$config_home" \
		...
		PATH="$data_home/kinetixos/scripts:$PATH" \
...
mkdir -p "$config_home/quickshell" "$config_home/kinetixos" \
	"$config_home/autostart" "$data_home/applications" \
	"$data_home/kinetixos/config" "$data_home/kinetixos/scripts" \
	"$state_home/kinetixos/appearance" \
...
cp "$repo/tests/fixtures/system-operation-provider.py" "$data_home/kinetixos/scripts/dwm-system-management"
chmod +x "$data_home/kinetixos/scripts/dwm-system-management"
cp "$repo/config/quickshell/assets/kinetixos_logo.png" "$home/Pictures/backgrounds/test-wallpaper.png"
...
cp "$repo/config/"*.toml "$config_home/kinetixos/"
cp "$repo/config/themes.toml" "$data_home/kinetixos/config/themes.toml"
printf '# inactive integration watch fixture\n' >"$config_home/kinetixos/theme-env.sh"
...
cp "$repo/scripts/dwm-settings-provider" "$repo/scripts/dwm-system-health" \
	...
	"$repo/scripts/dwm-terminal" "$repo/scripts/dwm-lock" "$data_home/kinetixos/scripts/"

input_discovery_fixture=$config_home/kinetixos/input-discovery-fixture
mv "$data_home/kinetixos/scripts/dwm-settings-input" \
	"$data_home/kinetixos/scripts/dwm-settings-input.real"
cat >"$data_home/kinetixos/scripts/dwm-settings-input" <<'SH'
#!/bin/sh
set -eu
fixture=$XDG_CONFIG_HOME/kinetixos/input-discovery-fixture
...
SH
chmod +x "$data_home/kinetixos/scripts/dwm-settings-input"

appearance_failure_fixture=$work/appearance-snapshot-failure
mv "$data_home/kinetixos/scripts/dwm-settings-appearance" \
	"$data_home/kinetixos/scripts/dwm-settings-appearance.real"
cat >"$data_home/kinetixos/scripts/dwm-settings-appearance" <<'SH'
#!/bin/sh
set -eu
...
SH
chmod +x "$data_home/kinetixos/scripts/dwm-settings-appearance"

mv "$data_home/kinetixos/scripts/dwm-settings-personalization" \
	"$data_home/kinetixos/scripts/dwm-settings-personalization.real"
cat >"$data_home/kinetixos/scripts/dwm-settings-personalization" <<'SH'
#!/bin/sh
set -eu
fixture=${DWM_SETTINGS_TEST_APPEARANCE_FAILURE:?}
...
SH
chmod +x "$data_home/kinetixos/scripts/dwm-settings-personalization"

wallpaper_status_fixture=$work/wallpaper-status
mv "$data_home/kinetixos/scripts/dwm-settings-wallpaper" \
	"$data_home/kinetixos/scripts/dwm-settings-wallpaper.real"
cat >"$data_home/kinetixos/scripts/dwm-settings-wallpaper" <<'SH'
#!/bin/sh
set -eu
...
SH
chmod +x "$data_home/kinetixos/scripts/dwm-settings-wallpaper"

theme_status_fixture=$work/theme-preview-status
mv "$data_home/kinetixos/scripts/dwm-settings-theme" \
	"$data_home/kinetixos/scripts/dwm-settings-theme.real"
cat >"$data_home/kinetixos/scripts/dwm-settings-theme" <<'SH'
#!/bin/sh
set -eu
...
SH
chmod +x "$data_home/kinetixos/scripts/dwm-settings-theme"

malformed_power_snapshot=$work/malformed-power-snapshot
mv "$data_home/kinetixos/scripts/dwm-quickshell-controlcenter" \
	"$data_home/kinetixos/scripts/dwm-quickshell-controlcenter.real"
cat >"$data_home/kinetixos/scripts/dwm-quickshell-controlcenter" <<'SH'
#!/bin/sh
set -eu
...
SH
chmod +x "$data_home/kinetixos/scripts/dwm-quickshell-controlcenter"
cat >"$data_home/kinetixos/scripts/kitty" <<'EOF'
#!/bin/sh
exit 0
EOF
cat >"$data_home/kinetixos/scripts/alacritty" <<'EOF'
#!/bin/sh
exit 0
EOF
chmod +x "$data_home/kinetixos/scripts/kitty" "$data_home/kinetixos/scripts/alacritty"

cat >"$data_home/kinetixos/scripts/dbus-monitor" <<'SH'
#!/bin/sh
...
SH
cat >"$data_home/kinetixos/scripts/light-locker" <<'SH'
#!/bin/sh
exit 0
SH
cat >"$data_home/kinetixos/scripts/nmcli" <<'SH'
#!/bin/sh
set -eu
while [ "$#" -gt 0 ]; do
	case $1 in
...
SH
cat >"$data_home/kinetixos/scripts/bluetoothctl" <<'SH'
#!/bin/sh
set -eu
case $* in
show)
...
SH
cat >"$data_home/kinetixos/scripts/busctl" <<'SH'
#!/bin/sh
set -eu
case $* in
'--user '*)
...
SH
chmod +x "$data_home/kinetixos/scripts/dbus-monitor" \
	"$data_home/kinetixos/scripts/light-locker" \
	"$data_home/kinetixos/scripts/nmcli" \
	"$data_home/kinetixos/scripts/bluetoothctl" \
	"$data_home/kinetixos/scripts/busctl"
...
cat >"$data_home/kinetixos/scripts/xset" <<'SH'
#!/bin/sh
set -eu
...
SH
chmod +x "$data_home/kinetixos/scripts/xset"
...
settings_power_watch_count() {
	...
	for monitor_pid in $(pgrep -f '[d]bus-monitor --system.*org.freedesktop.UPower.*org.freedesktop.UPower.PowerProfiles.*org.freedesktop.login1' || true); do
		...
		*"$data_home/kinetixos/scripts/dwm-quickshell-controlcenter power-watch"* | \
			*"$data_home/kinetixos/scripts/dwm-quickshell-controlcenter.real power-watch"*)
...
settings_autostart_watch_count() {
	expected="$data_home/kinetixos/scripts/dwm-xdg-autostart watch"
	count=0
...
while [ "$i" -lt 100 ]; do
	grep -Fqx "$sticky_record" \
		"$config_home/kinetixos/input-settings.conf" 2>/dev/null && break
	i=$((i + 1))
	sleep 0.05
done
grep -Fqx "$sticky_record" \
	"$config_home/kinetixos/input-settings.conf"
i=0
while [ "$i" -lt 100 ]; do
	preview_state=$(settings_ipc_retry inputPreviewState)
...
DISPLAY=$display HOME=$home XDG_CONFIG_HOME=$config_home XDG_RUNTIME_DIR=$runtime \
	"$data_home/kinetixos/scripts/dwm-settings-input" apply-saved
...
[ "$sticky_value" = "$sticky_baseline" ]
[ "$sticky_live" = "$sticky_baseline" ]
if [ -f "$config_home/kinetixos/input-settings.conf" ] &&
	grep -Fq "$(printf 'accessx\tsticky-keys\t')" \
		"$config_home/kinetixos/input-settings.conf"; then
...
if ! pgrep -af '[d]wm-quickshell-controlcenter([.]real)? power-dpms off$' |
	grep -F "$data_home/kinetixos/scripts/dwm-quickshell-controlcenter" >/dev/null; then
...
sed -i 's/terminal = "alacritty"/terminal = "kitty"/' "$config_home/kinetixos/hotkeys.toml"
i=0
while [ "$i" -lt 200 ]; do
...
sed -i 's/terminal = "kitty"/terminal = "alacritty"/' "$config_home/kinetixos/hotkeys.toml"
i=0
while [ "$i" -lt 200 ]; do
	terminal_id=$(DISPLAY=$display HOME=$home XDG_CONFIG_HOME=$config_home XDG_DATA_HOME=$data_home \
		XDG_RUNTIME_DIR=$runtime quickshell ipc --path "$config" call settings defaultsRoleDesktopId terminal 2>/dev/null || true)
...
[ "$defaults_busy" = false ]
[ "$terminal_id" = kitty.desktop ]
grep -Fqx 'terminal = "kitty"' "$config_home/kinetixos/hotkeys.toml"
...
[ "$defaults_busy" = false ]
[ "$terminal_id" = Alacritty.desktop ]
grep -Fqx 'terminal = "alacritty"' "$config_home/kinetixos/hotkeys.toml"
...
defaults_helper=$data_home/kinetixos/scripts/dwm-default-apps
...
autostart_helper=$data_home/kinetixos/scripts/dwm-xdg-autostart
...
*)
	...
	HOME=$home XDG_CONFIG_HOME=$config_home XDG_DATA_HOME=$data_home \
		DWM_SETTINGS_TEST_APPEARANCE_FAILURE="$appearance_failure_fixture" \
		"$data_home/kinetixos/scripts/dwm-settings-appearance" snapshot >&2 || true
	exit 1
	;;
esac
...
printf 'accessibility-settings-protocol	1	0\ncontrast	high\nmotion	reduced\n' |
	cmp - "$config_home/kinetixos/accessibility.conf"
...
notification_wait_available
python3 - "$config_home/kinetixos/notification-settings.json" <<'PYTHON'
import json
import sys
...
printf '%s\n' '{"version":2,"doNotDisturb":true,"popupTimeoutMs":1}' \
	>"$config_home/kinetixos/notification-settings.json"
i=0
while [ "$i" -lt 100 ]; do
	notification_policy_state=$(notification_ipc_retry policyState)
...
managed_panel_helper=$data_home/kinetixos/scripts/dwm-panel-settings
...
grep -Fqx "$expected" "$config_home/kinetixos/panel-widgets.conf"
DISPLAY=$display HOME=$home XDG_CONFIG_HOME=$config_home XDG_DATA_HOME=$data_home \
...
DISPLAY=$display HOME=$home XDG_CONFIG_HOME=$config_home XDG_DATA_HOME=$data_home \
	...
	DWM_SETTINGS_TEST_APPEARANCE_FAILURE=$appearance_failure_fixture \
	"$data_home/kinetixos/scripts/dwm-settings-personalization" \
...
[ -n "$test_font" ]
DISPLAY=$display HOME=$home XDG_CONFIG_HOME=$config_home XDG_DATA_HOME=$data_home \
	XDG_RUNTIME_DIR=$runtime "$data_home/kinetixos/scripts/dwm-settings-font" \
...
DISPLAY=$display HOME=$home XDG_CONFIG_HOME=$config_home XDG_DATA_HOME=$data_home \
	XDG_RUNTIME_DIR=$runtime "$data_home/kinetixos/scripts/dwm-settings-font" revert nested-font >/dev/null
i=0
while [ "$i" -lt 100 ]; do
	font_preview=$(DISPLAY=$display HOME=$home XDG_CONFIG_HOME=$config_home XDG_DATA_HOME=$data_home \
		XDG_RUNTIME_DIR=$runtime quickshell ipc --path "$config" call settings appearanceFontPreviewState 2>/dev/null || true)
...
DISPLAY=$display HOME=$home XDG_CONFIG_HOME=$config_home XDG_DATA_HOME=$data_home \
	XDG_RUNTIME_DIR=$runtime "$data_home/kinetixos/scripts/dwm-settings-font" reset >/dev/null
i=0
while [ "$i" -lt 100 ]; do
	font_scale=$(DISPLAY=$display HOME=$home XDG_CONFIG_HOME=$config_home XDG_DATA_HOME=$data_home \
		XDG_RUNTIME_DIR=$runtime quickshell ipc --path "$config" call settings appearanceFontScale 2>/dev/null || true)
...
DISPLAY=$display HOME=$home XDG_CONFIG_HOME=$config_home XDG_DATA_HOME=$data_home \
	...
	"$data_home/kinetixos/scripts/dwm-settings-wallpaper" apply "$test_wallpaper" max >/dev/null
i=0
while [ "$i" -lt 200 ]; do
...
DISPLAY=$display HOME=$home XDG_CONFIG_HOME=$config_home XDG_DATA_HOME=$data_home \
	XDG_RUNTIME_DIR=$runtime DWM_APPEARANCE_WALLPAPER_DIR=$home/Pictures/backgrounds \
	"$data_home/kinetixos/scripts/dwm-settings-wallpaper" \
...
if [ "$wallpaper_preview" != active ]; then
	...
	DISPLAY=$display HOME=$home XDG_CONFIG_HOME=$config_home XDG_DATA_HOME=$data_home \
		XDG_RUNTIME_DIR=$runtime DWM_APPEARANCE_WALLPAPER_DIR=$home/Pictures/backgrounds \
		"$data_home/kinetixos/scripts/dwm-settings-wallpaper" status --read-only >&2 || true
	exit 1
fi
...
wallpaper_preview_meta=$state_home/kinetixos/appearance/wallpaper/nested-wallpaper.meta
...
DISPLAY=$display HOME=$home XDG_CONFIG_HOME=$config_home XDG_DATA_HOME=$data_home \
	XDG_RUNTIME_DIR=$runtime DWM_APPEARANCE_WALLPAPER_DIR=$home/Pictures/backgrounds \
	"$data_home/kinetixos/scripts/dwm-settings-wallpaper" revert nested-wallpaper >/dev/null
i=0
while [ "$i" -lt 200 ]; do
	wallpaper_preview=$(DISPLAY=$display HOME=$home XDG_CONFIG_HOME=$config_home XDG_DATA_HOME=$data_home \
		XDG_RUNTIME_DIR=$runtime quickshell ipc --path "$config" call settings appearanceWallpaperPreviewState 2>/dev/null || true)
...
cp "$repo/config/quickshell/assets/kinetixos_logo.png" "$test_wallpaper"
...
transaction_state_file=$state_home/kinetixos/appearance/integration-transaction
...
if [ "$appearance_status" != partial ] ||
	...
	DISPLAY=$display HOME=$home XDG_CONFIG_HOME=$config_home XDG_DATA_HOME=$data_home \
		XDG_RUNTIME_DIR=$runtime DWM_SETTINGS_TEST_THEME_STATUS=$theme_status_fixture \
		DWM_SETTINGS_TEST_APPEARANCE_FAILURE=$appearance_failure_fixture \
		"$data_home/kinetixos/scripts/dwm-settings-personalization" status >&2 || true
	exit 1
fi
rm -f "$appearance_failure_fixture"
...
cp "$config_home/kinetixos/themes.toml" "$work/valid-themes.toml"
...
cp "$work/valid-themes.toml" "$config_home/kinetixos/themes.toml"
i=0
while [ "$i" -lt 100 ]; do
	appearance_theme=$(DISPLAY=$display HOME=$home XDG_CONFIG_HOME=$config_home XDG_DATA_HOME=$data_home \
		XDG_RUNTIME_DIR=$runtime quickshell ipc --path "$config" call settings appearanceActiveTheme 2>/dev/null || true)
...
mv "$config_home/kinetixos/themes.toml" "$work/named-themes.toml"
mv "$data_home/kinetixos/config/themes.toml" "$work/managed-themes.toml"
i=0
while [ "$i" -lt 200 ]; do
	appearance_status=$(DISPLAY=$display HOME=$home XDG_CONFIG_HOME=$config_home XDG_DATA_HOME=$data_home \
		XDG_RUNTIME_DIR=$runtime quickshell ipc --path "$config" call settings appearanceProviderStatus 2>/dev/null || true)
...
cat >"$config_home/kinetixos/themes.toml" <<'EOF'
[colors]
normfgcolor = "#D8DEE9"
normbgcolor = "#2E3440"
...
while [ "$i" -lt 100 ]; do
	if ! pgrep -af '[d]wm-settings-provider discover$' |
		grep -F "$data_home/kinetixos/scripts/dwm-settings-provider" >/dev/null; then
		break
	fi
	i=$((i + 1))
	sleep 0.05
...
if pgrep -af '[d]wm-settings-display watch [0-9]+ [0-9]+$' |
	grep -F "$data_home/kinetixos/scripts/dwm-settings-display" >/dev/null; then
	printf 'Settings display watcher remained active after close\n' >&2
	exit 1
fi
if pgrep -af '[d]wm-settings-input watch [0-9]+ [0-9]+$' |
	grep -F "$data_home/kinetixos/scripts/dwm-settings-input" >/dev/null; then
...
if pgrep -af '[d]wm-quickshell-network (snapshot|wifi-scan|wifi-connect|connect|disconnect|forget)' |
	grep -F "$data_home/kinetixos/scripts/dwm-quickshell-network" >/dev/null; then
...
if pgrep -af '[d]wm-quickshell-controls (bluetooth-snapshot|bluetooth-scan|bluetooth-power|bluetooth-pair|bluetooth-trust|bluetooth-connect|bluetooth-disconnect|bluetooth-remove)' |
	grep -F "$data_home/kinetixos/scripts/dwm-quickshell-controls" >/dev/null; then
...
if pgrep -af '[d]wm-default-apps (snapshot|watch|set-role|set-mime|reset-role|reset-mime)' |
	grep -F "$data_home/kinetixos/scripts/dwm-default-apps" >/dev/null; then
	printf 'Settings-owned Defaults work remained active after close\n' >&2
	exit 1
fi
if pgrep -af '[d]wm-xdg-autostart (snapshot|watch|set|reset)' |
	grep -F "$data_home/kinetixos/scripts/dwm-xdg-autostart" >/dev/null; then
