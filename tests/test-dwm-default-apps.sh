...
mkdir -p "$work/bin" "$work/fail-bin" "$work/data/applications/vendor" "$work/home/.config/kinetixos" \
...
cat >"$work/home/.config/kinetixos/hotkeys.toml" <<'EOF'
[vars]
terminal = "alacritty"
...
EOF
chmod 640 "$work/home/.config/kinetixos/hotkeys.toml"
...
recovery_backup_path() {
	local scope=$1
	local state_dir=$work/home/.local/state/kinetixos/default-apps
	local name
...
defaults_state=$work/home/.local/state/kinetixos/default-apps
...
hotkeys_before=$(sha256sum "$work/home/.config/kinetixos/hotkeys.toml")
hotkeys_without_terminal_before=$(sed '/^[[:space:]]*terminal[[:space:]]*=/d' \
	"$work/home/.config/kinetixos/hotkeys.toml" | sha256sum)
result=$(run_helper set-role terminal kitty.desktop)
[[ $result == $'defaults-result\t1\t0\tset-role\tterminal\tkitty.desktop\tok' ]]
grep -Fqx 'terminal = "kitty"' "$work/home/.config/kinetixos/hotkeys.toml"
[[ $(stat -c %a "$work/home/.config/kinetixos/hotkeys.toml") == 640 ]]
[[ $(sed '/^[[:space:]]*terminal[[:space:]]*=/d' "$work/home/.config/kinetixos/hotkeys.toml" | sha256sum) == "$hotkeys_without_terminal_before" ]]
result=$(run_helper reset-role terminal)
[[ $result == $'defaults-result\t1\t0\treset-role\tterminal\t\tok' ]]
[[ $(sha256sum "$work/home/.config/kinetixos/hotkeys.toml") == "$hotkeys_before" ]]

sed -i 's/terminal = "alacritty"/  terminal = "alacritty"  # preserve terminal choice/' \
	"$work/home/.config/kinetixos/hotkeys.toml"
commented_hotkeys_before=$(sha256sum "$work/home/.config/kinetixos/hotkeys.toml")
snapshot=$(run_helper snapshot)
grep -Fqx $'role\tterminal\tavailable\tAlacritty.desktop\tAlacritty\thotkeys.toml\tTerminal hotkey selection is readable' <<<"$snapshot"
result=$(run_helper set-role terminal kitty.desktop)
[[ $result == $'defaults-result\t1\t0\tset-role\tterminal\tkitty.desktop\tok' ]]
...
result=$(run_helper reset-role terminal)
[[ $result == $'defaults-result\t1\t0\treset-role\tterminal\t\tok' ]]
[[ $(sha256sum "$work/home/.config/kinetixos/hotkeys.toml") == "$commented_hotkeys_before" ]]

cp "$work/home/.config/kinetixos/hotkeys.toml" "$work/hotkeys.valid-comment"
sed -i 's/  terminal = .*/terminal = "alacritty" trailing = "injection" # invalid/' \
	"$work/home/.config/kinetixos/hotkeys.toml"
malformed_hotkeys_before=$(sha256sum "$work/home/.config/kinetixos/hotkeys.toml")
snapshot=$(run_helper snapshot)
grep -Fq $'role\tterminal\trestricted\t\t\thotkeys.toml\t' <<<"$snapshot"
expect_status 1 run_helper set-role terminal kitty.desktop
[[ $(sha256sum "$work/home/.config/kinetixos/hotkeys.toml") == "$malformed_hotkeys_before" ]]
cp "$work/hotkeys.valid-comment" "$work/home/.config/kinetixos/hotkeys.toml"
sed -i 's/  terminal = .*/terminal = "alacritty # unterminated/' \
	"$work/home/.config/kinetixos/hotkeys.toml"
unterminated_hotkeys_before=$(sha256sum "$work/home/.config/kinetixos/hotkeys.toml")
snapshot=$(run_helper snapshot)
grep -Fq $'role\tterminal\trestricted\t\t\thotkeys.toml\t' <<<"$snapshot"
expect_status 1 run_helper set-role terminal kitty.desktop
[[ $(sha256sum "$work/home/.config/kinetixos/hotkeys.toml") == "$unterminated_hotkeys_before" ]]
cp "$work/hotkeys.valid-comment" "$work/home/.config/kinetixos/hotkeys.toml"
sed -i '/terminal = /a terminal = "kitty" # duplicate' \
	"$work/home/.config/kinetixos/hotkeys.toml"
duplicate_hotkeys_before=$(sha256sum "$work/home/.config/kinetixos/hotkeys.toml")
snapshot=$(run_helper snapshot)
grep -Fq $'role\tterminal\trestricted\t\t\thotkeys.toml\t' <<<"$snapshot"
expect_status 1 run_helper set-role terminal kitty.desktop
[[ $(sha256sum "$work/home/.config/kinetixos/hotkeys.toml") == "$duplicate_hotkeys_before" ]]
mv "$work/hotkeys.valid-comment" "$work/home/.config/kinetixos/hotkeys.toml"
...
cp "$work/home/.config/kinetixos/hotkeys.toml" "$work/hotkeys.target"
rm "$work/home/.config/kinetixos/hotkeys.toml"
ln -s "$work/hotkeys.target" "$work/home/.config/kinetixos/hotkeys.toml"
snapshot=$(run_helper snapshot)
...
expect_status 1 run_helper set-role terminal kitty.desktop
rm "$work/home/.config/kinetixos/hotkeys.toml"
mv "$work/hotkeys.target" "$work/home/.config/kinetixos/hotkeys.toml"

mv "$work/home/.config/kinetixos" "$work/hotkeys-parent-target"
ln -s "$work/hotkeys-parent-target" "$work/home/.config/kinetixos"
...
rm "$work/home/.config/kinetixos"
mv "$work/hotkeys-parent-target" "$work/home/.config/kinetixos"

mkdir -p "$work/home/.local/state/kinetixos/default-apps/.lock"
expect_status 75 run_helper set-role terminal kitty.desktop
rmdir "$work/home/.local/state/kinetixos/default-apps/.lock"

mkdir -p "$work/home/.local/state/kinetixos/default-apps/.lock"
printf '999999 1\n' >"$work/home/.local/state/kinetixos/default-apps/.lock/owner"
result=$(run_helper set-role terminal kitty.desktop)
[[ $result == $'defaults-result\t1\t0\tset-role\tterminal\tkitty.desktop\tok' ]]
...
printf '%s %s\n' "$$" "$shell_starttime" \
	>"$work/home/.local/state/kinetixos/default-apps/.lock/owner"
expect_status 75 run_helper set-role terminal kitty.desktop
rm "$work/home/.local/state/kinetixos/default-apps/.lock/owner"
rmdir "$work/home/.local/state/kinetixos/default-apps/.lock"
...
if command -v inotifywait >/dev/null 2>&1; then
	...
	for _ in {1..100}; do
		...
		config_managed_child=$(watch_child_for_path "$watch_pid" \
			"$absent_config/kinetixos" 2>/dev/null || true)
	...
	printf '[vars]\nterminal = "alacritty"\n' >"$absent_config/kinetixos/hotkeys.toml"
	for _ in {1..100}; do
	...
	mkdir -p "$raced_config/kinetixos"
	printf '[vars]\nterminal = "alacritty"\n' >"$raced_config/kinetixos/hotkeys.toml"
	...
	env "${env_common[@]}" XDG_CONFIG_HOME="$raced_config" \
		...
		DWM_TEST_WATCH_SETUP_TARGET="$raced_config/kinetixos" \
	...
	mkdir "$raced_config/kinetixos"
	setup_managed_child=
	for _ in {1..100}; do
		setup_managed_child=$(watch_child_for_path "$watch_pid" \
			"$raced_config/kinetixos" 2>/dev/null || true)
