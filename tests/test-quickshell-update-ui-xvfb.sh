...
mkdir -p "$work/qml" "$work/home/.config/kinetixos" \
	"$work/data/kinetixos/scripts" "$work/runtime" "$work/state"
...
cp "$repo/config/"*.toml "$work/home/.config/kinetixos/"
cp "$repo/tests/qml/SystemUpdateUi.qml" "$work/qml/shell.qml"
cp "$repo/tests/fixtures/system-update-ui-provider.py" "$work/data/kinetixos/scripts/dwm-system-management"
chmod +x "$work/data/kinetixos/scripts/dwm-system-management"
