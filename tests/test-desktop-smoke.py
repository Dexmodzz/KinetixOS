...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix="qs-", dir=runtime_parent) as runtime, \
        ...
        (config / "kinetixos").mkdir()
        for source in (REPO / "config").glob("*.toml"):
            shutil.copy(source, config / "kinetixos")
        (data / "kinetixos").mkdir(parents=True)
        (data / "kinetixos/scripts").symlink_to(REPO / "scripts", target_is_directory=True)
