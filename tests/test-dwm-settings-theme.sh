...
home_dir=$work/home
themes_file=$config_home/kinetixos/themes.toml
managed_file=$data_home/kinetixos/config/themes.toml
apply_stub=$work/apply-theme
reload_stub=$work/reload-theme
xsettings_stub=$work/reload-xsettings

mkdir -p "$config_home/kinetixos" "$data_home/kinetixos/config" "$state_home" \
...
integration_snapshot() {
	local path
	for path in \
		"$config_home/kinetixos/personalization.conf" \
		...
		"$home_dir/.gtkrc-2.0" \
		"$config_home/kinetixos/cursor.Xresources" \
		"$config_home/kinetixos/theme-env.sh" \
		"$config_home/kinetixos/xsettingsd.conf" \
...
reset_fixture() {
	...
	mkdir -p "$config_home/kinetixos" "$state_home" "$runtime_dir" "$work/dconf" "$work/xfconf"
...
mkdir -p "$relative_home/.config/kinetixos" \
	"$relative_home/.local/share/kinetixos/config" "$relative_runtime"
cp "$repo/config/themes.toml" "$relative_home/.config/kinetixos/themes.toml"
cp "$repo/config/themes.toml" "$relative_home/.local/share/kinetixos/config/themes.toml"
(
	cd "$work"
...
grep -Fq 'theme = "dracula"' "$relative_home/.config/kinetixos/themes.toml"
...
[[ $(active_theme) == nord ]]
[[ ! -e $state_home/kinetixos/appearance/transaction.meta ]]
...
same_exchange_name=$(awk -F= '$1 == "exchange_file" { print $2; exit }' \
	"$state_home/kinetixos/appearance/transaction.meta")
...
grep -Fqx "$same_source_hash" \
	"$state_home/kinetixos/appearance/integration-suppress"
[[ $(active_theme) == nord ]]
...
slow_prefix=$state_home/kinetixos/appearance/slow-preview.preview
...
cancel_prefix=$state_home/kinetixos/appearance/cancelled-watchdog.preview
...
mkdir -p "$state_home/kinetixos/appearance/orphaned-claim.preview.claim"
wait_for_theme nord
grep -Fqx $'result\tnone' < <(run_theme preview-status)
...
chmod 640 "$config_home/alacritty/alacritty.toml" "$config_home/kitty/kitty.conf" \
	...
	"$config_home/kinetixos/cursor.Xresources" "$config_home/kinetixos/theme-env.sh" \
	"$config_home/qt6ct/qt6ct.conf"
...
integration_transaction=$state_home/kinetixos/appearance/integration-transaction
...
suppression_file=$state_home/kinetixos/appearance/integration-suppress
...
grep -Fqx '# external integration edit before publish' \
	"$config_home/alacritty/alacritty.toml"
[[ -f $state_home/kinetixos/appearance/integration-publish-race.preview.meta ]]
...
durable_prefix=$state_home/kinetixos/appearance/durable-preview.preview
...
reset_fixture
mkdir -p "$state_home/kinetixos/appearance"
printf 'orphan-preview\n' >"$state_home/kinetixos/appearance/preview.current"
grep -Fqx $'result\tnone' < <(run_theme preview-status)
[[ ! -e $state_home/kinetixos/appearance/preview.current ]]
...
[[ ! -d $state_home/kinetixos/appearance/stale-keep.preview.claim ]]
...
[[ ! -e $state_home/kinetixos/appearance/integration-transaction ]]
...
rm -f -- "$state_home/kinetixos/appearance/transaction.integration.11.before" \
	"$state_home/kinetixos/appearance/transaction.integration.11.after" \
	"$state_home/kinetixos/appearance/transaction.integration.11.meta" \
	"$state_home/kinetixos/appearance/transaction.integration.12.before" \
	"$state_home/kinetixos/appearance/transaction.integration.12.after" \
	"$state_home/kinetixos/appearance/transaction.integration.12.meta"
printf '11\n' >"$state_home/kinetixos/appearance/transaction.integration.count"
...
cp "$config_home/kinetixos/personalization.conf" "$work/personalization-malformed.before"
...
grep -Fqx $'personalization-protocol\t1\t0' \
	"$config_home/kinetixos/personalization.conf"
[[ $(wc -l <"$config_home/kinetixos/personalization.conf") == 1 ]]
...
grep -Fqx $'qt\tgtk3' "$config_home/kinetixos/personalization.conf"
grep -Fqx $'recovery\tnone' < <(run_theme recovery-status)
...
grep -Fqx '# unrelated edit after interrupted repair' \
	"$config_home/alacritty/active-theme.toml"
rm "$config_home/alacritty/active-theme.toml"
[[ ! -e $state_home/kinetixos/appearance/integration-suppress ]]
...
cp "$config_home/kinetixos/personalization.conf" "$work/personalization-duplicate.before"
...
printf 'personalization-protocol\t1\t0\nqt\tgtk3\n' \
	>"$config_home/kinetixos/personalization.conf"
ln "$config_home/kinetixos/personalization.conf" "$work/personalization-hardlink.conf"
if run_theme personalization-ready; then
...
rm "$config_home/kinetixos/personalization.conf"
run_theme personalization-ready
...
grep -Fqx 'broken-protocol' "$config_home/kinetixos/personalization.conf"
[[ $(active_theme) == dracula ]]
reset_fixture
...
reset_fixture
chmod 500 "$config_home/kinetixos"
if run_theme mutation-ready; then
	printf 'non-writable theme directory was reported mutable\n' >&2
	exit 1
fi
chmod 700 "$config_home/kinetixos"
rm "$themes_file"
...
fallback_output=$(HOME=$fallback_home XDG_CONFIG_HOME=$fallback_config XDG_DATA_HOME=$data_home \
	...
	DWM_TEST_LIVE_LOG=$work/live.log PATH=$work/integration-bin:$PATH \
	DWM_APPEARANCE_THEMES_FILE=$fallback_config/kinetixos/themes.toml \
	DWM_APPEARANCE_MANAGED_THEMES_FILE=$managed_file \
...
grep -Fqx '# Auto-generated by theme-apply.sh - do not edit manually.' \
	"$config_home/kinetixos/xsettingsd.conf"
grep -Fqx 'Keep/Setting "yes"' "$config_home/kinetixos/xsettingsd.conf"
grep -Fqx 'Xft/DPI 122880' "$config_home/kinetixos/xsettingsd.conf"
grep -Eq '^Net/ThemeName[[:space:]]' "$config_home/kinetixos/xsettingsd.conf"
...
grep -Fqx $'text-size\tfollow-system' "$config_home/kinetixos/personalization.conf"
grep -Fqx 'Keep/Setting "yes"' "$config_home/kinetixos/xsettingsd.conf"
if grep -Eq '^Xft/DPI[[:space:]]' "$config_home/kinetixos/xsettingsd.conf"; then
...
grep -Fqx $'personalization-protocol\t1\t0' \
	"$config_home/kinetixos/personalization.conf"
grep -Fqx $'font\tKeep Font' "$config_home/kinetixos/personalization.conf"
grep -Fqx $'qt\tgtk3' "$config_home/kinetixos/personalization.conf"
grep -Fqx 'export QT_QPA_PLATFORMTHEME=gtk3' "$config_home/kinetixos/theme-env.sh"
...
grep -Fqx $'gtk\tCustomXdg' "$config_home/kinetixos/personalization.conf"
...
grep -Fqx $'qt\tfollow-theme' "$config_home/kinetixos/personalization.conf"
...
grep -Fqx "'External Font 12'" "$work/dconf/font-name"
[[ ! -e $config_home/kinetixos/personalization.conf ]]
grep -Fqx $'recovery\tnone' < <(run_theme recovery-status)
...
cmp -s "$work/dconf/font-name" \
	"$state_home/kinetixos/appearance/transaction.gsettings.after"
...
[[ -f $config_home/kinetixos/xsettingsd.conf ]]
...
[[ ! -e $config_home/kinetixos/xsettingsd.conf ]]
grep -Fqx reload "$work/xsettings.log"
grep -Fqx $'recovery\tnone' < <(run_theme recovery-status)
