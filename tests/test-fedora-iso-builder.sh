...
standard_iso="$work/kinetixos.iso"
nvidia_iso="$work/kinetixos-nvidia.iso"
touch "$input_iso"
...
for failure in implant check; do
	...
	if compgen -G "$work/.kinetixos.iso.tmp.*" >/dev/null; then
...
for modern in (False, True):
    ...
    scripts = root / 'usr/share/kinetixos-image/scripts'
    (scripts / 'image').mkdir(parents=True)
    (root / 'etc').mkdir()
    (root / 'etc/kinetixos-image').touch()
    ...
    test_body = body.replace('/mnt/sysimage', str(root)).replace('/run/install/repo/kinetixos', str(repo))
