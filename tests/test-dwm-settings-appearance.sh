...
data_root=$work/data
bin_dir=$work/bin
mkdir -p "$config_home/kinetixos" "$config_home/alacritty" "$config_home/kitty" \
...
cp "$repo/config/themes.toml" "$config_home/kinetixos/themes.toml"
...
before_hash=$(sha256sum "$config_home/kinetixos/themes.toml")
before_mode=$(stat -c %a "$config_home/kinetixos/themes.toml")
output=$(snapshot)
after_hash=$(sha256sum "$config_home/kinetixos/themes.toml")
after_mode=$(stat -c %a "$config_home/kinetixos/themes.toml")
...
grep -Fqx $'source\tuser\t'"$config_home/kinetixos/themes.toml" <<<"$output"
...
cp "$config_home/kinetixos/themes.toml" "$work/replacement-themes.toml"
...
immutable_snapshot=$(PATH=$bin_dir GTK_THEME='' XCURSOR_THEME='' \
	...
	DWM_TEST_REPLACEMENT_TARGET="$config_home/kinetixos/themes.toml" \
	DWM_TEST_REPLACED_MARKER="$work/replaced.marker" "$helper" snapshot)
grep -Fqx $'active\tnord\tnord\tselected' <<<"$immutable_snapshot"
grep -Fq 'theme = "dracula"' "$config_home/kinetixos/themes.toml"
rm -f "$bin_dir/awk"
...
mv "$config_home/kinetixos/theme-env.sh" "$work/theme-env.sh"
...
grep -Fqx $'error\tqt\tmissing-backend\tNo applied Qt theme backend is recorded' \
	<<<"$no_qt"
mv "$work/theme-env.sh" "$config_home/kinetixos/theme-env.sh"

mv "$config_home/kinetixos/theme-env.sh" "$work/theme-env-regular.sh"
mkfifo "$config_home/kinetixos/theme-env.sh"
...
grep -Fqx $'integration\tqt\tavailable\tqt6ct\tQt applications use the supported theme backend' \
	"$work/special-theme-env.out"
rm "$config_home/kinetixos/theme-env.sh"
mv "$work/theme-env-regular.sh" "$config_home/kinetixos/theme-env.sh"
...
rm -f "$config_home/kinetixos/personalization.conf"
...
sed -i 's/^\[active\]$/[active] # selected theme/' "$config_home/kinetixos/themes.toml"
sed -i 's/^\[theme\.nord\]$/[theme.nord] # default theme/' "$config_home/kinetixos/themes.toml"
...
sed -i 's/^\[theme\.dracula\]$/[theme.dracula] trailing/' "$config_home/kinetixos/themes.toml"
...
cp "$repo/config/themes.toml" "$config_home/kinetixos/themes.toml"
sed -i 's/^\[active\]$/[active/' "$config_home/kinetixos/themes.toml"
...
mv "$config_home/kinetixos/themes.toml" "$work/managed-themes.toml"
...
mkdir -p "$data_root/kinetixos/config"
cp "$work/managed-themes.toml" "$data_root/kinetixos/config/themes.toml"
installed_managed=$(PATH=$bin_dir XDG_CONFIG_HOME=$config_home XDG_DATA_HOME=$data_root \
	DWM_APPEARANCE_DATA_DIRS=$data_root "$helper" snapshot)
grep -Fqx $'source\tmanaged\t'"$data_root/kinetixos/config/themes.toml" <<<"$installed_managed"

mkdir "$config_home/kinetixos/themes.toml"
set +e
...
grep -Fqx $'provider\tappearance\tunavailable\tread-only\tShared theme inventory and integration state' \
	<<<"$blocked_user"
grep -Fqx $'source\tuser\t'"$config_home/kinetixos/themes.toml" <<<"$blocked_user"
...
grep -Fq $'error\tsource\tunreadable\tUser theme file is not a readable regular file: ' \
	<<<"$blocked_user"
rmdir "$config_home/kinetixos/themes.toml"

ln -s "$work/missing-user-theme.toml" "$config_home/kinetixos/themes.toml"
dangling_user=$(snapshot)
grep -Fqx $'provider\tappearance\tpartial\tread-only\tShared theme inventory and integration state' \
	<<<"$dangling_user"
grep -Fqx $'source\tmanaged\t'"$data_root/kinetixos/config/themes.toml" <<<"$dangling_user"
...
grep -Fq $'error\tsource\tdangling-user\tIgnoring dangling user theme symlink and trying the managed source: ' \
	<<<"$dangling_user"
rm -f "$config_home/kinetixos/themes.toml"

custom_theme=-custom_theme_with_a_name_that_is_longer_than_sixty_four_characters_1234567890
cp "$work/managed-themes.toml" "$config_home/kinetixos/themes.toml"
sed -i "0,/theme = \"nord\"/s//theme = \"$custom_theme\"/" "$config_home/kinetixos/themes.toml"
sed -i "0,/^\[theme\.nord\]$/s//[theme.$custom_theme]/" "$config_home/kinetixos/themes.toml"
custom=$(snapshot)
...
cp "$work/managed-themes.toml" "$config_home/kinetixos/themes.toml"
sed -i '0,/theme = "nord"/s//theme = 123/' "$config_home/kinetixos/themes.toml"
...
cp "$work/managed-themes.toml" "$config_home/kinetixos/themes.toml"
sed -i "0,/theme = \"nord\"/s//theme = \"$overlong_theme\"/" "$config_home/kinetixos/themes.toml"
sed -i "0,/^\[theme\.nord\]$/s//[theme.$overlong_theme]/" "$config_home/kinetixos/themes.toml"
overlong=$(snapshot)
...
cp "$work/managed-themes.toml" "$config_home/kinetixos/themes.toml"
sed -i '0,/theme = "nord"/s//theme = "dracula"/' "$config_home/kinetixos/themes.toml"
...
cp "$work/managed-themes.toml" "$config_home/kinetixos/themes.toml"
sed -i '0,/theme = "nord"/s//theme = "missing"/' "$config_home/kinetixos/themes.toml"
unknown=$(snapshot)
...
cp "$work/managed-themes.toml" "$config_home/kinetixos/themes.toml"
sed -i '0,/theme = "nord"/s//theme = "broken"/' "$config_home/kinetixos/themes.toml"
printf '\n[theme.broken]\ndark_mode = true\n' >>"$config_home/kinetixos/themes.toml"
incomplete=$(snapshot)
...
grep -Fqx $'theme\tchosen\tselected\tvalid\ttrue\tNordic\tTheme record is complete' \
	<<<"$selected_after_cap"
sed -i '0,/theme = "chosen"/s//theme = chosen/' "$config_home/kinetixos/themes.toml"
...
mkdir -p "$work/fallback-home/.config/kinetixos"
cp "$work/managed-themes.toml" "$work/fallback-home/.config/kinetixos/themes.toml"
relative_xdg=$(HOME="$work/fallback-home" XDG_CONFIG_HOME=relative-config \
	...
	"$helper" snapshot)
grep -Fqx $'source\tuser\t'"$work/fallback-home/.config/kinetixos/themes.toml" \
	<<<"$relative_xdg"
grep -Fqx $'active\tnord\tnord\tselected' <<<"$relative_xdg"

rm -f "$config_home/kinetixos/themes.toml" "$work/missing-managed.toml"
set +e
