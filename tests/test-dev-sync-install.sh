...
mkdir -p "$test_repo" "$test_bin" "$prefix/bin" "$prefix/libexec/kinetixos" \
	"$manprefix/man1" "$xsessions_dir" \
	"$data_root/icons" "$data_root/licenses/kinetixos/capitaine-cursors" \
...
sed "s|@PREFIX@|$prefix|g" "$test_repo/scripts/dwm-settings-display-root" |
	install -Dm755 /dev/stdin "$prefix/libexec/kinetixos/dwm-settings-display-root"
sed "s|@PREFIX@|$prefix|g" "$test_repo/scripts/dwm-desktop-update-root" |
	install -Dm755 /dev/stdin "$prefix/libexec/kinetixos/dwm-desktop-update-root"
