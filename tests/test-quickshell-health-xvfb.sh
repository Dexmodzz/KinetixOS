...
mkdir -p "$config_home/quickshell" "$config_home/kinetixos" "$data_home/kinetixos/scripts" "$runtime"
chmod 700 "$runtime"
...
sed -i '/title="dwm control center utility"/d' "$config_home/kinetixos/window-rules.toml"
sed -i '/title="dwm network password"/a\
  { title="dwm control center",         isfloating=1, alwaysontop=1 },' \
	"$config_home/kinetixos/window-rules.toml"
...
while [ "$i" -lt 200 ]; do
	...
	if ! printf '%s\n' "$scan_processes" | grep -F "$data_home/kinetixos/scripts/dwm-system-health" >/dev/null; then
		break
	fi
	i=$((i + 1))
	sleep 0.05
