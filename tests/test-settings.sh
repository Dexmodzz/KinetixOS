...
mkdir -p "$work/fedora-config/kinetixos"
cp "$repo/config/themes.toml" "$work/fedora-config/kinetixos/themes.toml"
...
mkdir -p "$repo_source_home" "$repo_source_config/kinetixos"
cp "$repo/config/themes.toml" "$repo_source_config/kinetixos/themes.toml"
...
mkdir -p "$work/home/.config/quickshell" "$work/home/.local/share/kinetixos/config/quickshell" "$work/launcher-bin"
...
grep -Fq 'the previous KinetixOS next-login layout will be backed up' \
	"$repo/config/quickshell/settings/DisplaySettingsPane.qml"
