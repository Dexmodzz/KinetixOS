...
bin=$work/bin
mkdir -p "$config_home/quickshell" "$config_home/kinetixos" "$home/.cache" \
	"$data_home/kinetixos/scripts" "$data_home/applications" "$runtime" "$bin"
...
cp "$repo/scripts/dwm-settings-provider" "$repo/scripts/dwm-system-health" \
	...
	"$repo/scripts/dwm-lock" "$data_home/kinetixos/scripts/"
...
env DISPLAY="$display" HOME="$home" XDG_CONFIG_HOME="$config_home" \
	...
	DWM_LARGE_SURFACE_DEX_LOG="$work/dex.log" PATH="$bin:$data_home/kinetixos/scripts:$PATH" \
...
if ! grep -Eq '"popupTimeoutMs"[[:space:]]*:[[:space:]]*4000' \
	"$config_home/kinetixos/notification-settings.json"; then
	printf 'Notification popup timeout was not persisted:\n' >&2
	sed -n '1,20p' "$config_home/kinetixos/notification-settings.json" >&2
	exit 1
fi
...
[ "$(ipc notifications doNotDisturb)" = true ]
[ "$(ipc notifications popupTimeout)" = 4000 ]
chmod 000 "$config_home/kinetixos/notification-settings.json"
touch "$config_home/kinetixos/notification-settings.json"
i=0
while [ "$i" -lt 100 ]; do
	[ "$(ipc notifications policyState)" = unavailable ] && break
	i=$((i + 1))
...
chmod 600 "$config_home/kinetixos/notification-settings.json"
touch "$config_home/kinetixos/notification-settings.json"
i=0
while [ "$i" -lt 100 ]; do
	[ "$(ipc notifications policyState)" = available ] && break
...
printf '%s\n' '{malformed' >"$config_home/kinetixos/notification-settings.json"
i=0
while [ "$i" -lt 100 ]; do
