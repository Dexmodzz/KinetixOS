...
if command -v Xvfb >/dev/null 2>&1 && [ -x "$repo/dwm" ]; then
	...
	mkdir -p "$runtime_data_home/kinetixos/scripts" \
		"$runtime_data_home/kinetixos/config"
	cp "$repo/config/hotkeys.toml" "$repo/config/themes.toml" \
		"$repo/config/window-rules.toml" "$runtime_data_home/kinetixos/config/"
	cat >"$runtime_data_home/kinetixos/scripts/autostart.sh" <<'SH'
#!/bin/sh
exit 0
SH
	cat >"$runtime_data_home/kinetixos/scripts/autostop.sh" <<'SH'
#!/bin/sh
: >"${DWM_SESSION_TEST_AUTOSTOP_MARKER:?}"
SH
	cat >"$runtime_data_home/kinetixos/scripts/theme-apply.sh" <<'SH'
#!/bin/sh
printf x >>"${DWM_SESSION_TEST_THEME_APPLY_MARKER:?}"
printf '%s\n%s\n' "$XDG_CONFIG_HOME" "$XDG_DATA_HOME" \
	>"${DWM_SESSION_TEST_THEME_ENV_MARKER:?}"
SH
	chmod +x "$runtime_data_home/kinetixos/scripts/"*.sh
	...
	mkdir -p "$runtime_config_home/kinetixos"
	cp "$repo/config/themes.toml" "$runtime_config_home/kinetixos/themes.toml"
	...
	printf '\n' >>"$runtime_config_home/kinetixos/themes.toml"
	i=0
	while [ "$(grep -Fc 'dwm: loaded theme from config' "$work/dwm.log" || true)" \
