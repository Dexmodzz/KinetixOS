...
mkdir -p "$work/bin" "$work/home/.config/kinetixos" "$work/runtime"
...
grep -Fqx $'accessx\tsticky-keys\t1\t0' \
	"$work/home/.config/kinetixos/input-settings.conf"
printf '0 0 0 1 0\n' >"$work/xkbset.state"
env "${env_common[@]}" "$helper" apply-saved
grep -Fqx '0 1 0 1 0' "$work/xkbset.state"
env "${env_common[@]}" "$helper" reset accessx sticky-keys >/dev/null
grep -Fqx '0 0 0 1 0' "$work/xkbset.state"
if [[ -f $work/home/.config/kinetixos/input-settings.conf ]] &&
	grep -Fq $'accessx\tsticky-keys\t' "$work/home/.config/kinetixos/input-settings.conf"; then
...
if [[ -f $work/home/.config/kinetixos/input-settings.conf ]] &&
	grep -Fq "$nodeless_key" "$work/home/.config/kinetixos/input-settings.conf"; then
...
cp "$work/home/.config/kinetixos/input-settings.conf" "$work/saved-connected"
{
...
if grep -Fq "$unstable_mouse_key" "$work/home/.config/kinetixos/input-settings.conf"; then
...
if grep -Fq "$keyboard_key"$'\tkeyboard-layout\t' \
	"$work/home/.config/kinetixos/input-settings.conf"; then
