...
state=$fixture/state
bin=$fixture/bin
font_config=$config/kinetixos/font.conf
...
grep -Fqx $'family\tInter' "$home/.config/kinetixos/font.conf"
grep -Fqx $'scale\t1.10' "$home/.config/kinetixos/font.conf"
...
[[ ! -e $font_config ]]
[[ ! -e $state/kinetixos/appearance/font/preview.current ]]

export DWM_TEST_FONT_DEFAULT_ALIAS=1
status=$(run_font status)
grep -Fqx $'selection\tavailable\tMesloLGS Nerd Font Mono\t1.00\tManaged shell font defaults are active' <<<"$status"
run_font apply 'MesloLGS Nerd Font Mono' 1.00 >/dev/null
grep -Fqx $'family\tMesloLGS Nerd Font Mono' "$config/kinetixos/font.conf"
...
[[ $(stat -c %a "$font_config") == 640 ]]
[[ -e $state/kinetixos/appearance/font/preview.current ]]
[[ -e $state/kinetixos/appearance/font/preview.baseline ]]
status=$(run_font status)
...
[[ $(sha256sum "$font_config" | awk '{ print $1 }') == "$baseline_hash" ]]
i=0
while [[ -e $state/kinetixos/appearance/font/preview.current && $i -lt 80 ]]; do
	((i += 1))
	sleep 0.1
done
[[ ! -e $state/kinetixos/appearance/font/preview.current ]]
...
awk -F '\t' -v pid="$$" -v start="$shell_start" \
	...
	"$state/kinetixos/appearance/font/preview.meta" >"$work/reboot.meta"
mv "$work/reboot.meta" "$state/kinetixos/appearance/font/preview.meta"
...
font_state_dir=$state/kinetixos/appearance/font
...
watchdog_lock=$state/kinetixos/appearance/font/mutation.lock
...
exec 8>&-
i=0
while [[ -e $state/kinetixos/appearance/font/preview.current && $i -lt 100 ]]; do
	((i += 1))
	sleep 0.1
done
[[ ! -e $state/kinetixos/appearance/font/preview.current ]]
...
cleanup_watchdog_pid=$(awk -F '\t' '$1 == "watchdog-pid" { print $2 }' \
	"$state/kinetixos/appearance/font/preview.meta")
cleanup_children=''
for _ in {1..50}; do
