...
grep -Fqx $'mutation\trestricted\tWallpaper changes are unavailable in this session' \
	<<<"$read_only_initial"
test ! -e "$config_home/kinetixos"
test ! -e "$state_home/kinetixos"
...
test ! -e "$state_home/kinetixos/appearance/wallpaper/mutation.failed"
...
test ! -e "$state_home/kinetixos/appearance/wallpaper/preview.current"
test ! -e "$state_home/kinetixos/appearance/wallpaper/mutation.failed"
...
multi_monitor_meta=$state_home/kinetixos/appearance/wallpaper/multi-monitor-baseline.meta
...
test "$(grep -c '^path=' "$state_home/kinetixos/appearance/wallpaper/session.default")" -eq 2
printf '0\n' >"$loadable_count_file"
...
mkdir -p "$config_home/kinetixos"
chmod 750 "$config_home/kinetixos"
...
fit=max
EOF
cmp "$work/expected.conf" "$config_home/kinetixos/wallpaper.conf"
test "$(stat -c %a "$config_home/kinetixos/wallpaper.conf")" = 600
test "$(stat -c %a "$config_home/kinetixos")" = 750
...
cp -- "$config_home/kinetixos/wallpaper.conf" "$work/before-randomize.conf"
...
cmp "$work/before-randomize.conf" "$config_home/kinetixos/wallpaper.conf"
grep -Fqx "path=$second" "$state_home/kinetixos/appearance/wallpaper/session.default"
...
grep -Fqx "arg=$first" "$log"
grep -Fqx 'arg=--bg-max' "$log"
test ! -e "$state_home/kinetixos/appearance/wallpaper/session.default"
...
grep -Fqx "xauthority=$relative_session/.Xauthority" \
	"$state_home/kinetixos/appearance/wallpaper/relative-authority.meta"
: >"$log"
run_helper revert relative-authority >/dev/null
grep -Fqx "xauthority=$relative_session/.Xauthority" "$log"
grep -Fqx "path=$first" "$config_home/kinetixos/wallpaper.conf"
grep -Fqx 'fit=max' "$config_home/kinetixos/wallpaper.conf"
...
test ! -s "$log"
preview_meta=$state_home/kinetixos/appearance/wallpaper/keep-me.meta
...
grep -Fqx "path=$second" "$config_home/kinetixos/wallpaper.conf"
grep -Fqx "arg=$second" "$log"
...
grep -Fqx 'fit=tile' "$config_home/kinetixos/wallpaper.conf"
...
grep -Fqx $'selection\tpartial\t'"$second"$'\ttile\tConfigured wallpaper could not be applied; the legacy random wallpaper fallback is required' \
	<<<"$degraded_status"
test -f "$state_home/kinetixos/appearance/wallpaper/selection.owner"
test ! -e "$state_home/kinetixos/appearance/wallpaper/selection.failed"
grep -Fqx 'version=3' "$state_home/kinetixos/appearance/wallpaper/selection.owner"
...
grep -Fqx "baseline_path=$first" \
	"$state_home/kinetixos/appearance/wallpaper/degraded-baseline.meta"
...
grep -Fqx $'selection\tavailable\t'"$second"$'\ttile\tManaged wallpaper file is readable; image decoding is checked when applied' \
	<<<"$ready_status"
test ! -e "$state_home/kinetixos/appearance/wallpaper/selection.failed"
test ! -e "$state_home/kinetixos/appearance/wallpaper/selection.owner"
...
rearm_meta=$state_home/kinetixos/appearance/wallpaper/rearm-me.meta
...
cat >"$config_home/kinetixos/wallpaper.conf" <<EOF
version=1
path=$second
fit=center
EOF
...
test ! -e "$state_home/kinetixos/appearance/wallpaper/preview.current"
grep -Fqx "path=$second" "$config_home/kinetixos/wallpaper.conf"
grep -Fqx 'fit=center' "$config_home/kinetixos/wallpaper.conf"
...
ln -s "$symlink_failure_target" \
	"$state_home/kinetixos/appearance/wallpaper/symlinked-failure.failed"
cat >"$config_home/kinetixos/wallpaper.conf" <<EOF
version=1
path=$first
fit=max
...
test ! -L "$state_home/kinetixos/appearance/wallpaper/symlinked-failure.failed"
test "$(stat -c %h "$state_home/kinetixos/appearance/wallpaper/symlinked-failure.failed")" = 1
...
ln "$hardlink_failure_target" \
	"$state_home/kinetixos/appearance/wallpaper/hardlinked-failure.failed"
cat >"$config_home/kinetixos/wallpaper.conf" <<EOF
version=1
path=$second
fit=center
...
test "$(stat -c %h "$state_home/kinetixos/appearance/wallpaper/hardlinked-failure.failed")" = 1
...
external_expiry_meta=$state_home/kinetixos/appearance/wallpaper/external-expiry.meta
...
printf '%s\n' 'Configured and fallback wallpapers are unavailable' \
	>"$state_home/kinetixos/appearance/wallpaper/failed-abandon.failed"
rm -f -- "$second"
...
printf 'second image\n' >"$second"
export DWM_TEST_FEH_MUTATE_CONFIG=$config_home/kinetixos/wallpaper.conf
...
grep -Fqx 'version=1' "$config_home/kinetixos/wallpaper.conf"
grep -Fqx 'mode=default' "$config_home/kinetixos/wallpaper.conf"
...
printf '%s\n' 'Wallpaper live rollback failed; retry session recovery before previewing' \
	>"$state_home/kinetixos/appearance/wallpaper/mutation.failed"
chmod 600 "$state_home/kinetixos/appearance/wallpaper/mutation.failed"
export DWM_TEST_FEH_FAIL_ALL=1
reset_output=$(run_helper reset)
grep -Fqx $'result\treset\tunavailable' <<<"$reset_output"
test -f "$state_home/kinetixos/appearance/wallpaper/mutation.failed"
...
export DWM_TEST_DEFAULT_CANDIDATE=$first
run_helper session-apply
grep -Fqx "path=$first" "$state_home/kinetixos/appearance/wallpaper/session.default"
...
rm -f -- "$config_home/kinetixos/wallpaper.conf"
ln -s "$work/unsafe.conf" "$config_home/kinetixos/wallpaper.conf"
printf '%s\n' 'Wallpaper configuration became unsafe during preview' \
	>"$state_home/kinetixos/appearance/wallpaper/unsafe-config-abandon.failed"
chmod 600 "$state_home/kinetixos/appearance/wallpaper/unsafe-config-abandon.failed"
if run_helper mutation-ready; then
...
test ! -e "$state_home/kinetixos/appearance/wallpaper/preview.current"
test -L "$config_home/kinetixos/wallpaper.conf"
...
printf 'not a wallpaper config\n' >"$config_home/kinetixos/wallpaper.conf"
...
grep -Fqx 'not a wallpaper config' "$config_home/kinetixos/wallpaper.conf"
test ! -s "$log"
rm -f -- "$config_home/kinetixos/wallpaper.conf"
...
preview_current=$state_home/kinetixos/appearance/wallpaper/preview.current
...
EOF
chmod +x "$bin_dir/mv"
export DWM_TEST_REAL_MV=$real_mv DWM_TEST_CONFIG_FILE=$config_home/kinetixos/wallpaper.conf
export DWM_TEST_SESSION_DEFAULT=$state_home/kinetixos/appearance/wallpaper/session.default
...
test ! -e "$config_home/kinetixos/wallpaper.conf"
...
unset DWM_TEST_MV_MODE
test -d "$config_home/kinetixos/wallpaper.conf"
test -z "$(find "$config_home/kinetixos/wallpaper.conf" -mindepth 1 -print -quit)"
grep -Fq 'could not be published' "$work/directory-publish.err"
rmdir "$config_home/kinetixos/wallpaper.conf"
...
grep -Fqx "path=$second" "$config_home/kinetixos/wallpaper.conf"
grep -Fqx 'fit=tile' "$config_home/kinetixos/wallpaper.conf"
recovery_configs=("$config_home"/kinetixos/wallpaper.conf.staged.*)
...
test ! -e "$state_home/kinetixos/appearance/wallpaper/session.default"
test -z "$(find "$state_home/kinetixos/appearance/wallpaper" -maxdepth 1 \
	-name '.session-default.*' -print -quit)"
test ! -e "$state_home/kinetixos/appearance/wallpaper/mutation.owner"
...
test ! -e "$preview_current"
test ! -e "$state_home/kinetixos/appearance/wallpaper/interrupted-publish.meta"
...
[[ -s $preview_publish_ready ]]
killed_before_publish_meta=$state_home/kinetixos/appearance/wallpaper/killed-before-publish.meta
...
test ! -e "$state_home/kinetixos/appearance/wallpaper/preview.owner"
test ! -e "$preview_current"
...
test -f "$state_home/kinetixos/appearance/wallpaper/mutation.failed"
test -f "$state_home/kinetixos/appearance/wallpaper/mutation.owner"
recovery_status=$(run_helper status)
grep -Fqx $'mutation\trestricted\tWallpaper live rollback failed; retry session recovery before previewing' \
	<<<"$recovery_status"
...
cat >"$state_home/kinetixos/appearance/wallpaper/mutation.owner" <<'EOF'
version=1
token=mutation
display=:915
xauthority=
EOF
chmod 600 "$state_home/kinetixos/appearance/wallpaper/mutation.owner"
...
if compgen -G "$config_home/kinetixos/wallpaper.conf.staged.*" >/dev/null; then
...
display_preview_meta=$state_home/kinetixos/appearance/wallpaper/display-owned.meta
...
foreign_rearm_meta=$state_home/kinetixos/appearance/wallpaper/foreign-rearm.meta
...
reused_meta=$state_home/kinetixos/appearance/wallpaper/reused-watchdog-pid.meta
...
stale_display_meta=$state_home/kinetixos/appearance/wallpaper/stale-display.meta
...
test ! -e "$state_home/kinetixos/appearance/wallpaper/preview.current"
test ! -e "$state_home/kinetixos/appearance/wallpaper/transient-session-recovery.meta"
test ! -e "$state_home/kinetixos/appearance/wallpaper/transient-session-recovery.failed"
...
backward_clock_meta=$state_home/kinetixos/appearance/wallpaper/backward-clock.meta
...
fractional_deadline_meta=$state_home/kinetixos/appearance/wallpaper/fractional-deadline.meta
...
resume_deadline_meta=$state_home/kinetixos/appearance/wallpaper/resume-deadline.meta
grep -Fqx 'deadline=1120000' "$resume_deadline_meta"
: >"$log"
printf '1130.00 0.00\n' >"$fake_monotonic"
for _ in {1..60}; do
	[[ ! -e $state_home/kinetixos/appearance/wallpaper/preview.current ]] && break
	sleep 0.05
done
test ! -e "$state_home/kinetixos/appearance/wallpaper/preview.current"
...
stale_boot_meta=$state_home/kinetixos/appearance/wallpaper/stale-boot.meta
...
failed_display_meta=$state_home/kinetixos/appearance/wallpaper/failed-display.meta
...
printf '%s\n' 'Owner display rollback failed' \
	>"$state_home/kinetixos/appearance/wallpaper/failed-display.failed"
: >"$log"
run_helper_display :916 session-apply
test ! -e "$state_home/kinetixos/appearance/wallpaper/preview.current"
grep -Fqx 'display=:915' "$log"
...
reused_display_meta=$state_home/kinetixos/appearance/wallpaper/reused-display-failure.meta
...
printf '%s\n' 'Previous session rollback failed' \
	>"$state_home/kinetixos/appearance/wallpaper/reused-display-failure.failed"
run_helper_display :915 session-apply
test ! -e "$state_home/kinetixos/appearance/wallpaper/preview.current"
...
test ! -e "$state_home/kinetixos/appearance/wallpaper/preview.current"
test ! -e "$state_home/kinetixos/appearance/wallpaper/bounded-feh.failed"
...
test ! -e "$state_home/kinetixos/appearance/wallpaper/preview.current"
test ! -e "$state_home/kinetixos/appearance/wallpaper/metadata-write-failure.meta"
if grep -Fqx "arg=$second" "$log"; then
...
finish_clamp_meta=$state_home/kinetixos/appearance/wallpaper/finish-clamp-failure.meta
...
clamp_meta=$state_home/kinetixos/appearance/wallpaper/reconcile-clamp-failure.meta
...
rearm_failure_meta=$state_home/kinetixos/appearance/wallpaper/reconcile-rearm-failure.meta
...
mktemp_failure_meta=$state_home/kinetixos/appearance/wallpaper/rearm-mktemp-failure.meta
...
noncanonical_meta=$state_home/kinetixos/appearance/wallpaper/noncanonical-deadline.meta
...
corrupt_active_meta=$state_home/kinetixos/appearance/wallpaper/corrupt-active-meta.meta
...
corrupt_owner_meta=$state_home/kinetixos/appearance/wallpaper/corrupt-current-owner.meta
...
: >"$state_home/kinetixos/appearance/wallpaper/preview.current"
: >"$log"
...
mismatched_owner_meta=$state_home/kinetixos/appearance/wallpaper/mismatched-owner-token.meta
...
stale_owner_meta=$state_home/kinetixos/appearance/wallpaper/corrupt-current-stale-owner.meta
...
missing_current_meta=$state_home/kinetixos/appearance/wallpaper/missing-current-after-apply.meta
...
rm -f -- "$state_home/kinetixos/appearance/wallpaper/preview.current"
: >"$log"
...
grep -Fqx $'preview\tfailed\tmissing-current-after-apply\t0\t\tfill\tWallpaper preview state is invalid; restore it explicitly before changing wallpaper' \
	<<<"$owner_only_status"
test -f "$state_home/kinetixos/appearance/wallpaper/preview.owner"
...
overlapping_owner_meta=$state_home/kinetixos/appearance/wallpaper/overlapping-owner-recovery.meta
...
rm -f -- "$state_home/kinetixos/appearance/wallpaper/preview.current"
selection_owner=$state_home/kinetixos/appearance/wallpaper/selection.owner
selection_hash=$(sha256sum "$config_home/kinetixos/wallpaper.conf" | awk '{ print $1 }')
...
unset DWM_TEST_FEH_FAIL_DISPLAY
grep -Fqx 'display=:915' "$state_home/kinetixos/appearance/wallpaper/selection.owner"
...
unset DWM_TEST_FEH_FAIL_DISPLAY
test "$(grep -Fxc 'display=:915' "$state_home/kinetixos/appearance/wallpaper/selection.owner")" -eq 1
test "$(grep -Fxc 'display=:916' "$state_home/kinetixos/appearance/wallpaper/selection.owner")" -eq 1
test "$(stat -c %s "$state_home/kinetixos/appearance/wallpaper/selection.owner")" -gt 8192
unset XAUTHORITY
cat >"$bin_dir/xprop" <<'EOF'
#!/bin/sh
[ "${1:-}" = -display ] && [ "${2:-}" = :915 ]
...
test -f "$state_home/kinetixos/appearance/wallpaper/selection.owner"
test "$(grep -Fxc 'display=:915' "$state_home/kinetixos/appearance/wallpaper/selection.owner")" -eq 1
test "$(grep -Fxc 'display=:916' "$state_home/kinetixos/appearance/wallpaper/selection.owner")" -eq 0
run_helper_display :916 apply "$first" tile >/dev/null
test ! -e "$state_home/kinetixos/appearance/wallpaper/selection.failed"
test -f "$state_home/kinetixos/appearance/wallpaper/selection.owner"
...
selection_failure=$state_home/kinetixos/appearance/wallpaper/selection.failed
selection_owner=$state_home/kinetixos/appearance/wallpaper/selection.owner
sha256sum "$config_home/kinetixos/wallpaper.conf" | awk '{ print $1 }' >"$selection_failure"
cat >"$selection_owner" <<'EOF'
version=2
display=:915
xauthority=
...
cp -- "$config_home/kinetixos/wallpaper.conf" "$saved_selection_config"
rm -f -- "$config_home/kinetixos/wallpaper.conf"
...
test ! -e "$config_home/kinetixos/wallpaper.conf"
mv -- "$saved_selection_config" "$config_home/kinetixos/wallpaper.conf"

run_helper preview missing-active-meta 20 "$second" fill >/dev/null
missing_active_meta=$state_home/kinetixos/appearance/wallpaper/missing-active-meta.meta
...
cp -- "$config_home/kinetixos/wallpaper.conf" "$config_before_malformed_owner"
...
cmp -- "$config_before_malformed_owner" "$config_home/kinetixos/wallpaper.conf"
grep -Fqx 'not a valid token' "$state_home/kinetixos/appearance/wallpaper/preview.current"
test ! -s "$log"
