...
work=$(mktemp -d)
custom_prefix=$(mktemp -d /opt/kinetixos-security-test.XXXXXX)
...
trap cleanup EXIT
mkdir -p "$custom_prefix/bin" "$custom_prefix/libexec/kinetixos" \
	"$work/bin" "$work/home/display-profiles"

installed="$custom_prefix/libexec/kinetixos/dwm-settings-display-root"
...
chmod 0777 "$custom_prefix/libexec/kinetixos"
if env DISPLAY=:99 HOME="$work/home" DWM_DISPLAY_PROFILE_DIR="$work/home/display-profiles" \
	PATH="$work/bin:$custom_prefix/bin:/usr/bin:/bin" \
...
chmod 0755 "$custom_prefix/libexec/kinetixos"

mv "$custom_prefix/libexec/kinetixos" "$work/real-libexec"
ln -s "$work/real-libexec" "$custom_prefix/libexec/kinetixos"
if env DISPLAY=:99 HOME="$work/home" DWM_DISPLAY_PROFILE_DIR="$work/home/display-profiles" \
	PATH="$work/bin:$custom_prefix/bin:/usr/bin:/bin" \
...
rm -f "$custom_prefix/libexec/kinetixos"
mv "$work/real-libexec" "$custom_prefix/libexec/kinetixos"
