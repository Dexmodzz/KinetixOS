...
class Security(unittest.TestCase):
    def setUp(self):
        ...
        self.helper = self.prefix / "libexec/kinetixos/dwm-desktop-update-root"
        ...
        self.manifest_path = self.prefix / "share/kinetixos/desktop-install.json"
        self.manifest_path.parent.mkdir(parents=True)
        self.manifest = {"schema": 1, "source": "https://github.com/Dexmodzz/KinetixOS.git",
        ...
        self.addCleanup(shutil.rmtree, Path("/var/lib/kinetixos/desktop-updates") / self.operation, True)
    ...
    def test_qml_bootstrap_rejects_path_shadowing_and_untrusted_installation(self):
        ...
        status = home / "state/kinetixos/desktop-update/status.json"
...
with open('/var/lib/kinetixos/desktop-updates/lock', 'a') as lock:
    try:
...
""")
        ...
        with Path("/var/lib/kinetixos/desktop-updates/lock").open("a") as lock:
            import fcntl
            fcntl.flock(lock, fcntl.LOCK_EX | fcntl.LOCK_NB)
...
""")
        ...
        self.assertEqual(guard("", ["/usr/bin/python3", str(probe), str(state / "lock"),
                                    "/var/lib/kinetixos/desktop-updates/lock"], "nobody", str(state)), 17)
    ...
    def test_dependency_action_rejects_unfinished_transactions_before_package_commands(self):
        ...
        generation = hashlib.sha256(self.manifest_path.read_bytes()).hexdigest()
        directory = Path("/var/lib/kinetixos/desktop-updates") / self.operation
    ...
    def test_live_uninstall_refuses_unfinished_transactions(self):
        generation = hashlib.sha256(self.manifest_path.read_bytes()).hexdigest()
        self.assertEqual(self.command("begin", generation, self.operation).returncode, 0)
        journal = Path("/var/lib/kinetixos/desktop-updates") / self.operation / "journal.json"
    ...
    def test_preparation_can_be_canceled_without_replacing_files(self):
        ...
        self.addCleanup(shutil.rmtree, Path("/var/lib/kinetixos/desktop-updates") / later, True)
    ...
    def test_root_can_run_read_only_installation_audits(self):
        ...
        state = self.prefix / "state/kinetixos/desktop-update"
        state.mkdir(parents=True)
        roots = [self.prefix / "config/quickshell", self.prefix / "data/kinetixos/config", self.prefix / "data/kinetixos/scripts"]
    ...
    def test_old_recovery_record_with_special_mode_is_rejected(self):
        self.archive()
        self.assertEqual(self.apply().returncode, 0)
        backup = Path("/var/lib/kinetixos/desktop-updates") / self.operation
    ...
    def test_newer_manual_install_prevents_interrupted_rollback(self):
        self.archive()
        self.assertEqual(self.apply().returncode, 0)
        journal_path = Path("/var/lib/kinetixos/desktop-updates") / self.operation / "journal.json"
    ...
    def test_one_session_installs_and_completes_with_protocol_output_only(self):
        ...
        self.assertEqual(self.binary.read_bytes(), b"updated")
        journal = json.loads((Path("/var/lib/kinetixos/desktop-updates") / self.operation / "journal.json").read_text())
