...
mkdir -p "$config_home/kinetixos" "$config_home/gtk-3.0" "$config_home/gtk-4.0" \
...
expected=$(printf 'provider\tpersonalization\tpartial\tuser-session\tPersisted personalization protocol is unsupported; use a compatible KinetixOS version')
...
expected=$(printf 'watch-readiness\ttext-size\tavailable\tManaged XSETTINGS owner lifecycle is observable')
printf '%s\n' "$stale_status" | grep -Fqx "$expected"
head -c 4097 /dev/zero | tr '\0' x >"$config_home/kinetixos/theme-env.sh"
